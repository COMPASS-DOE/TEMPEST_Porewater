...
year4_start = lubridate::as_date("2025-01-01")
year4_stop = lubridate::as_date("2025-12-31")
```
...
inventory_directory <- "https://docs.google.com/spreadsheets/d/1403K8StN0iwPnDXt0u-eLLxcfAS2Mid75rNfU9qojLw/edit?gid=581935863#gid=581935863"

drive_download(file = inventory_directory, path = "Data from Google Drive/TEMPEST_PorewaterInventory_May2022_Present_updated.xlsx", overwrite = TRUE)

sheet_names <- excel_sheets("Data from Google Drive/TEMPEST_PorewaterInventory_May2022_Present_updated.xlsx")
sheet_names

sample_key <- readxl::read_excel("Data from Google Drive/TEMPEST_PorewaterInventory_May2022_Present_updated.xlsx", sheet = "Porewater - Individual", skip = 4) %>%
  select(Sample_ID, Evacuation_date_YYYYMMDD, Collection_Date_YYYYMMDD, Collection_End_Time_24hrs, EST_EDT) %>%
  filter(str_detect(Sample_ID, "DOC")) %>%
  rename(sample_name = Sample_ID,
         evacuation_date = Evacuation_date_YYYYMMDD,
...
#Run these to pull the pH data into GitHub if not already there
directory <- "https://docs.google.com/spreadsheets/d/1-KTABMU-2e6ZxQXeY1yMahHB5lX9HuvN-NF-kWinLTA/edit?gid=1955793989#gid=1955793989"

drive_download(file = directory, path = "Data from Google Drive/TEMPEST_Porewater_pH_updated.xlsx", overwrite = TRUE)

sheet_names <- excel_sheets("Data from Google Drive/TEMPEST_Porewater_pH_updated.xlsx")
sheet_names

# pH_raw <- readxl::read_excel("~/GitHub/TEMPEST-1-porewater/data_do_not_commit/porewater_pH_raw.xlsx", sheet="pH") 

pH_raw <- readxl::read_excel("Data from Google Drive/TEMPEST_Porewater_pH_updated.xlsx", sheet="pH") 
pH_raw$Time_24hr <- as.integer(pH_raw$Time_24hr)

pH_annual <- pH_raw %>% 
  filter(!str_detect(Date_YYYYMMDD, "2026"))

pH_NA <- pH_annual %>% 
  filter(Cond_units == "NA" | is.na(Cond_units)) %>% 
  filter(!is.na(Cond))
...
pH <- pH_annual %>%
  ...
  filter(!is.na(pH)) %>%
  mutate(time = sprintf("%04d", as.integer(time)),
  ...
  mutate(Instrument = case_when(
    str_detect(Notes, "YSI") ~ "YSI",
    TRUE ~ "Oakton")) %>% 
  select(Site, plot, grid, pH, Cond, Cond_units, collection_temp_C, Instrument, datetime, Date_YYYYMMDD, Time_24hr, tz, flag, Notes)
...
porewater_pH_2023 <- porewater_pH %>% filter(datetime < year2_stop & datetime > year2_start) %>%
  select(Site, plot, grid, pH, Cond, Cond_units, collection_temp_C, Instrument, datetime, Date_YYYYMMDD, Time_24hr, tz, evacuation_date, flag)
 
porewater_pH_2024 <- porewater_pH %>% filter(datetime < year3_stop & datetime > year3_start) %>%
  select(Site, plot, grid, pH, Cond, Cond_units, collection_temp_C, Instrument, datetime, Date_YYYYMMDD, Time_24hr, tz, evacuation_date, flag)

porewater_pH_2025 <- porewater_pH %>% filter(datetime < year4_stop & datetime > year4_start) %>%
  select(Site, plot, grid, pH, Cond, Cond_units, collection_temp_C, Instrument, datetime, Date_YYYYMMDD, Time_24hr, tz, evacuation_date, flag)

# write_csv(porewater_pH_2023, "~/GitHub/TEMPEST-1-porewater/processed data/for_google_drive/TMP_PW_pH_2023_L1.csv")
# write_csv(porewater_pH_2024, "~/GitHub/TEMPEST-1-porewater/processed data/for_google_drive/TMP_PW_pH_2024_L1.csv")
# write_csv(porewater_pH_2025, "~/GitHub/TEMPEST-1-porewater/processed data/for_google_drive/TMP_PW_pH_2025_L1.csv")

write_csv(porewater_pH_2023, "TMP_PW_pH_2023_L1.csv")
write_csv(porewater_pH_2024, "TMP_PW_pH_2024_L1.csv")
write_csv(porewater_pH_2025, "TMP_PW_pH_2025_L1.csv")
...
source_pH_2023 <- pH %>%
  ...
  select(Site, plot, grid, pH, Cond, Cond_units, collection_temp_C, Instrument, datetime, Date_YYYYMMDD, Time_24hr, tz, flag)

source_pH_2024 <- pH %>%
  filter(grid == "SOURCE") %>%
  filter(datetime < year3_stop & datetime > year3_start) %>%
  select(Site, plot, grid, pH, Cond, Cond_units, collection_temp_C, Instrument, datetime, Date_YYYYMMDD, Time_24hr, tz, flag)

# no source in 2025 # 

# write_csv(source_pH_2023, "~/GitHub/TEMPEST-1-porewater/processed data/for_google_drive/TMP_SOURCE_pH_2023_L1.csv")
# write_csv(source_pH_2024, "~/GitHub/TEMPEST-1-porewater/processed data/for_google_drive/TMP_SOURCE_pH_2024_L1.csv")

write_csv(source_pH_2023, "TMP_SOURCE_pH_2023_L1.csv")
write_csv(source_pH_2024, "TMP_SOURCE_pH_2024_L1.csv")
```
