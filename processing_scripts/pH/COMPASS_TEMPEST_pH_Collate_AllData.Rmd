...
#read in 2023 pH Data:
dat23_pw <- read.csv("Raw Data/TMP_PW_pH_2023_L1.csv")
dat23_sw <- read.csv("Raw Data/TMP_SOURCE_pH_2023_L1.csv")
dat23 <- bind_rows(dat23_pw, dat23_sw)
colnames(dat23)

#read in 2024 pH Data:
dat24_pw <- read.csv("Raw Data/TMP_PW_pH_2024_L1.csv")
dat24_sw <- read.csv("Raw Data/TMP_SOURCE_pH_2024_L1.csv")
dat24 <- bind_rows(dat24_pw, dat24_sw)
colnames(dat24)

#read in 2025 pH Data:
...
raw_dat$Time_formatted <- format(strptime(sprintf("%04d", raw_dat$Time_24hr), format="%H%M"), format="%H:%M")
raw_dat$Date_formatted <- format(strptime(raw_dat$Date_YYYYMMDD, format="%Y%m%d"), format = "%Y-%m-%d")
raw_dat$Collection_Date_Time <- paste(raw_dat$Date_formatted, raw_dat$Time_formatted, sep = " ")
                                 
raw_dat$Collection_Date_Time <- as.POSIXct(raw_dat$Collection_Date_Time, format = "%Y-%m-%d %H:%M", tz = "US/Eastern")
...
all_dat <- raw_dat %>%
  mutate(
    Project = "TEMPEST", 
    Sample_ID = ifelse(grid != "SOURCE", 
                       paste0("TMP_", Zone, "_", grid, "_", Date_YYYYMMDD, "_", Time_24hr, "_15CM"), 
                       paste0("TMP_", Zone, "_", "SOURCEWATER", "_", Date_YYYYMMDD, "_", Time_24hr, "_15CM"))) %>%
  rename(
    Grid = grid, 
    Evacuation_Date = evacuation_date, 
    temperature_flag = flag
  ) %>% 
  select(Project, Sample_ID, Year, Month, Day, Zone, Grid, pH, Cond, Cond_units, Instrument, collection_temp_C, tz, Date_YYYYMMDD, Time_24hr, Evacuation_Date, temperature_flag, temperature_flag, pH_flag) 
...
metadat$Description <- c("Project name", 
                         ...
                         "Time of sample collection (HHMM)", 
                         "Date of sample evacuation",
                         "Flag to indicate if temperature was not recorded", 
                         "Flag to indicate if the pH is outside of the typical range")
...
write.csv(all_dat, "COMPASS_TEMPEST_pH_AllData.csv", row.names = FALSE)
...
data_plotting <- raw_dat %>% 
  filter(grid != "SOURCE") %>% 
  mutate(Zone_1 = factor(plot, levels = c("Control", "Freshwater", "Saltwater")))
...
##Boxplot Summarize pH by Date, Site, and Zone
viz_pH_boxplot <- ggplot(data_plotting, aes(x = grid,  y = pH, fill = Zone_1)) +
  geom_boxplot() + 
  facet_wrap( ~ Zone_1, scales="free_y", ncol = 1) +
  scale_x_discrete(drop = TRUE) +
  scale_fill_manual(values = c(
  ...
  theme_classic() +
  labs(x = "Grid ID", y = "pH", title = "TEMPEST Porewater: pH by Grid") +
  theme(
    panel.spacing = unit(0.5, "lines"), 
    panel.border = element_rect(color = "black", fill = NA, linewidth = 1)) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))+ 
