...
author: "2025 Samples Ran on 20251110"
date: "`r Sys.Date()`"
output: 
  pdf_document:
    toc: false
...
run_notes="Some sample IDs are missing from metadata: TMP_SW_PW_H6_20250926_15CM
Removed std 5 because it was low and none of the sample absorbances are above Std 3. 
Matrix checks were high, probably because std 5 was low. 
Dup and spk were good.
50 uL spike used"#any notes from run

#Stds that should be excluded
stds_to_remove<-data.frame(Plate=c("Plate4"),IDs=c("Std 5"))
# stds_to_remove<-NA
...
final_data_path <- "TEMPEST_H2S_20251110_Plates_Combined.csv"
HighCV_path <- "TEMPEST_H2S_20251110_Plates_HighCVs.csv"
ADL_not_fixed_path <- "TEMPEST_H2S_20251110_Plates_ADL.csv"
BDL_not_fixed_path <- "TEMPEST_H2S_20251110_Plates_BDL.csv"

#QAQC log path
log_path="Raw Data/Sulfide_STD_QAQC.csv"

#Sample ID Path
SampleIDs1=paste0("Raw Data/TEMPEST_2025_SampleIDs_20251110.xlsx")
...
for(i in 1:length(plates)){
  ...
  std5 <- subset(H2S_stds_all_fixed, IDs == "Std 5"& Plate ==std_curve$Plate)
  ...
  if("MC: 10ppt S5" %in% MC10$IDs){
    ...
    if(t.test.MC10$p.value > p_value_MC){
      ...
      print("Matrix Check 10 is significantly different from Std 5 - REASSESS")
    }
...
#Write out final data frame 
write.csv(final_data, "Processed Data/TEMPEST_H2S_AllPlates_Combined_20251110.csv", row.names = FALSE)

#Write out samples that still have high CVs
write.csv(HighCV, "Processed Data/TEMPEST_H2S_AllPlates_HighCVs_20251110.csv", row.names = FALSE)

#Write out ADL samples
write.csv(ADL_not_fixed, "Processed Data/TEMPEST_H2S_AllPlates_ADL_20251110.csv", row.names = FALSE)

#Write out BDL samples
write.csv(BDL_not_fixed, "Processed Data/TEMPEST_H2S_AllPlates_BDL_20251110.csv", row.names = FALSE)
