...
dat23 <- dat23 %>% 
  separate(Sample_ID, into = c("Collection_Date", "Zone", "Grid"), remove = FALSE)
# head(dat24)
colnames(dat23)
...
raw_dat$Grid <- gsub("Source", "Sourcewater", raw_dat$Grid)
raw_dat$Grid <- gsub("SOURCEWATER", "Sourcewater", raw_dat$Grid)
raw_dat$Grid <- gsub("RAINWATER", "Rainwater", raw_dat$Grid)
...
all_dat <- raw_dat %>%
  ...
  dplyr::select(
         ...
         SO4_ppm, SO4_mM, SO4_Conc_flag, SO4_QAQC_flag, Cl_ppm, Cl_mM, Cl_Conc_flag, Cl_QAQC_flag, 
         salinity, Analysis_rundate,  Run_notes, Field_notes
