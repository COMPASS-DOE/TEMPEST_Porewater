---
title: "TEMPEST CB: Porewater Sulfide"
author: "March-May 2026 Samples"
date: "`r Sys.Date()`"
output: 
  pdf_document:
    toc: false
...
file_paths_tidydata <- list.files(path = "Tidy Data", pattern = "tidy.*\\.csv$", full.names = TRUE)
(file_paths_tidydata)

#file path name for processed data
#Write out final data frame 
final_data_path <- "Processed Data/COMPASS_H2S_202603-05_Plates_Combined.csv"
HighCV_path <- "Processed Data/COMPASS_H2S_202603-05_Plates_HighCVs.csv"
ADL_not_fixed_path <- "Processed Data/COMPASS_H2S_202603-05_Plates_ADL.csv"
BDL_not_fixed_path <- "Processed Data/COMPASS_H2S_202603-05_Plates_BDL.csv"

#QAQC log path
log_path="Raw Data/Sulfide_STD_QAQC.csv"

#Metadata path
Raw_Metadata = "Raw Data/COMPASS_TEMPEST_PW_SampleLog_2026.csv"
...
dat <- dat %>%
  mutate(Plate = str_extract(Plate, "\\d{4}\\d{2}\\d{2}_Plate[0-9]+"))
...
dat$IDs[dat$IDs == "TMP_FW_B4_202604_15cm"] <- "TMP_SW_B4_202604_15cm"
...
H2S <- ggplot(H2S_stds, aes(Conc, Abs)) + geom_point() + geom_smooth(method = "lm", se = FALSE)+ 
  ...
  stat_regline_equation(aes(label = ..adj.rr.label..), label.x = 15, label.y = 1.2) # Add R squared
H2S
...
##write to QAQC log
updated_qlog <- rbind(qlogO, std_curve)
write.csv(updated_qlog, log_path)
...
HighCV<-filter(dat1, H2S_cv_flag=="High CV")

#filter out the samples that need to be run at a higher dilution
ADL<-filter(dat1, H2S_flag=="adl")

#remove the samples that have already been run at a higher dilution 
ww_ADL<-subset(dat1, (IDs %in% ADL$IDs))
ww_ADL<-filter(ww_ADL, H2S_flag=="Within Range") 
ADL_not_fixed<-ADL %>% 
...
BDL<-filter(dat1, H2S_flag=="bdl"& Dilution!=1)

#remove the samples that have already been run at a lower dilution 
ww_BDL<-subset(dat1, (IDs %in% BDL$IDs))
ww_BDL<-filter(ww_BDL,H2S_flag=="Within Range") 
BDL_not_fixed<-BDL %>% 
...
ADL_fixed<-ADL %>% 
  ...
  subset((!IDs %like% " Dup")) %>% 
  subset((!IDs %like% " Spike"))
ADL_fixed$ID_conc <- paste(ADL_fixed$IDs, ADL_fixed$H2S_flag, sep = "_")

BDL_fixed<-BDL %>% 
  subset((IDs %in% ww_BDL$IDs)) %>% 
  subset((!IDs %like% " Dup")) %>% 
  subset((!IDs %like% " Spike"))
BDL_fixed$ID_conc <- paste(BDL_fixed$IDs, BDL_fixed$H2S_flag, sep = "_")


dat1$ID_conc <- paste(dat1$IDs, dat1$H2S_flag, sep = "_")
...
```{r Remove dups, spks, and TMP samples, include=FALSE}

##Remove Dups and Spikes
Sample_data_IDs <- Sample_data_nodups %>% 
  subset((!IDs %like% "Dup")) %>% 
  subset((!IDs %like% "Spike"))%>% 
  subset((IDs %like% "TMP"))
...
#TMP_C_B4_202603_15cm to TMP_SW_PW_B4_20260410_15CM
##Separate sample IDs into columns 
...
####Fix Dates - ONLY FOR 202603 and 202304!####
Sample_data_IDs <- Sample_data_IDs %>% 
  mutate(Collection_Date = ifelse(Collection_Date == "202603", "20260309", Collection_Date)) %>% 
  mutate(Collection_Date = ifelse(Collection_Date == "202604", "20260410", Collection_Date)) %>% 
  mutate(Collection_Date = ifelse(Collection_Date == "202506", "20260511", Collection_Date)) %>% 
  mutate(Collection_Date = ifelse(Collection_Date == "202605", "20260508", Collection_Date)) %>% 
  mutate(Collection_Date = ifelse(IDs == "TMP_C_H6_202506_30cm", "20260511", Collection_Date)) %>% 
  mutate(Collection_Date = ifelse(IDs == "TMP_C_H3_202605_15cm", "20260511", Collection_Date)) %>% 
  mutate(Collection_Date = ifelse(IDs == "TMP_FW_F4_202605_30cm", "20260511", Collection_Date)) %>% 
  mutate(Collection_Date = ifelse(IDs == "TMP_FW_H6_202605_15cm", "20260511", Collection_Date))


####All of these are Sourcewater Samples
Sample_data_IDs <- Sample_data_IDs %>% 
  mutate(Source = "PW")
...
all_present <- all(Sample_data_IDs$Sample_ID %in% sulfide_metadata$H2S_ID)
...
if (all_present) {
  ...
  missing_ids <- setdiff(Sample_data_IDs$Sample_ID, sulfide_metadata$H2S_ID)
  print(missing_ids)
}
...
sulfide_metadata$Collection_Date <- as.character(sulfide_metadata$Collection_Date)

#merge metadata with sample run data 
merged_data <- Sample_data_IDs %>%
  left_join(sulfide_metadata, by = c("Sample_ID" = "H2S_ID", "Grid", "Collection_Date", "Project", "Zone", "Source", "Depth"))
...
final_data <- merged_data %>% 
  ...
  dplyr::select(
    Project, Sample_ID, Zone, Grid, Depth_cm,
         Source, Volume_mL, Collection_Date, Evacuation_date_YYYYMMDD, 
