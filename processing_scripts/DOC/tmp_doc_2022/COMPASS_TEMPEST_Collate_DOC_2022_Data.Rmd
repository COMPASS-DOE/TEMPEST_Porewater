...
sw_data$time1 <- substr(sw_data$datetime, 12, 16)
sw_data$time1 <- gsub(":", "", sw_data$time1)
sw_data$time = as.numeric(sw_data$time1) - 400
sw_data$time <- as.character(sw_data$time)
...
##For event samples, use T# designation 
all_data_event <- all_data %>%  
  filter(str_detect(sample_name, "_T")) 

all_data_event$"Timepoint" = substr(all_data_event$sample_name, nchar(all_data_event$sample_name) - 1, nchar(all_data_event$sample_name))


##For 20220518, 20220520 need to change times to T1-T3 in Sample ID
all_data_nonevent_special <- all_data %>%  
  filter(Collection_Date_YYYYMMDD == "20220518" | Collection_Date_YYYYMMDD == "20220520") 

all_data_nonevent_special <- all_data_nonevent_special %>% 
...
##For Sourcewater, need to change times to T0-T7 in Sample ID
all_data_sw <- all_data %>%  
  filter(Grid == "Source") 

all_data_sw <- all_data_sw %>% 
  group_by(Plot, Grid, Collection_Date_YYYYMMDD) %>% 
  arrange(Collection_End_Time_24hrs, .by_group = TRUE) %>% # Sort by Time ascending
  mutate(Timepoint = paste0("T", row_number()-1)) %>% 
  ungroup()

all_data_sw$Timepoint <- gsub("T6", "T7", all_data_sw$Timepoint)


##separate dataframe for the rest of the samples
all_data_other <- all_data %>%  
  filter(!str_detect(sample_name, "_T")) %>%  
  filter(Collection_Date_YYYYMMDD != "20220518" & Collection_Date_YYYYMMDD != "20220520" &  Grid != "Source") 

all_data_other$"Timepoint" = NA


##Combine dataframes
all_data2 <- rbind(all_data_event, all_data_nonevent_special, all_data_sw, all_data_other)

##Make new sample IDs
all_data2 <- all_data2 %>% 
  mutate(Sample_ID = paste(Site, Plot, Grid, Collection_Date_YYYYMMDD, Timepoint, sep = "_"))

all_data2$Sample_ID <- gsub("_NA", "", all_data2$Sample_ID)
...
## Fix Sample IDs
```{r}
##Remove TMP_FW_C3_20220520_T1 - no data, not in metadata
all_data3 <- subset(all_data3, Sample_ID != "TMP_FW_C3_20220520_T1")

##Fix TMP_FW_E3_20220518_T1 
all_data3$Sample_ID <- gsub("TMP_FW_E3_20220518_T1", "TMP_FW_E3_20220520_T1", all_data3$Sample_ID)
all_data3$Collection_Date_YYYYMMDD <- ifelse(all_data3$Sample_ID == "TMP_FW_E3_20220520_T1", "20220520", all_data3$Collection_Date_YYYYMMDD)

##Fix TMP_FW_E3_20220518_T1 
all_data3$Sample_ID <- gsub("TMP_FW_E3_20220518_T1", "TMP_FW_E3_20220520_T1", all_data3$Sample_ID)
all_data3$Collection_Date_YYYYMMDD <- ifelse(all_data3$Sample_ID == "TMP_FW_E3_20220520_T1", "20220520", all_data3$Collection_Date_YYYYMMDD)

##Fix TMP_FW_B4_20220718  
all_data3$Sample_ID <- gsub("TMP_FW_B4_20220718", "TMP_FW_B4_20220721", all_data3$Sample_ID)
all_data3$Collection_Date_YYYYMMDD <- ifelse(all_data3$Sample_ID == "TMP_FW_B4_20220721", "20220721", all_data3$Collection_Date_YYYYMMDD)

##Fix TMP_FW_F4_20220915  
all_data3$Sample_ID <- gsub("TMP_FW_F4_20220915", "TMP_FW_F4_20220912", all_data3$Sample_ID)
all_data3$Collection_Date_YYYYMMDD <- ifelse(all_data3$Sample_ID == "TMP_FW_F4_20220912", "20220912", all_data3$Collection_Date_YYYYMMDD)

##Fix TMP_C_C6_20221019   
all_data3$Sample_ID <- gsub("TMP_C_C6_20221019", "TMP_C_C6_20221024", all_data3$Sample_ID)
all_data3$Collection_Date_YYYYMMDD <- ifelse(all_data3$Sample_ID == "TMP_C_C6_20221024", "20221024", all_data3$Collection_Date_YYYYMMDD)
...
```{r pull in non-event and sourcewater metadata for later, include=FALSE}
...
raw_metadata_lys_year <- raw_metadata_lys_year %>%
  separate(
    col = Sample_ID,
    sep = "_",
    into = c("Site", "Plot", "Source" ,"Grid", "Depth", "Collection_Date", "Timepoint"),
    remove = FALSE)


##Change "1of3" to T1
raw_metadata_lys_year$Timepoint <- gsub("1of1", "T1", raw_metadata_lys_year$Timepoint)
raw_metadata_lys_year$Timepoint <- gsub("1of2", "T1", raw_metadata_lys_year$Timepoint)
raw_metadata_lys_year$Timepoint <- gsub("1of3", "T1", raw_metadata_lys_year$Timepoint)

raw_metadata_lys_year$Timepoint <- gsub("2of2", "T2", raw_metadata_lys_year$Timepoint)
raw_metadata_lys_year$Timepoint <- gsub("2of3", "T2", raw_metadata_lys_year$Timepoint)

raw_metadata_lys_year$Timepoint <- gsub("3of3", "T3", raw_metadata_lys_year$Timepoint)
...
raw_metadata_sw_year$Collection_Time_24hrs <- gsub(":", "", raw_metadata_sw_year$time)
...
doc_metadata_nonevent <- bind_rows(raw_metadata_lys_ids, raw_metadata_sw_combined)

#remove old ID's
doc_metadata_nonevent$Sample_ID <- NULL

#Rename columns that have missing data
doc_metadata_nonevent <- doc_metadata_nonevent %>%
  rename(Grid_meta = Grid, 
         Plot_meta = Plot, 
         Collection_End_Time_24hrs_meta = Collection_End_Time_24hrs, 
         Collection_Time_24hrs_meta = Collection_Time_24hrs, 
         Collection_Date_YYYYMMDD_meta = Collection_Date_YYYYMMDD)


colnames(doc_metadata_nonevent)

# head(doc_metadata)

```

##Pull in 2022 Event Metadata
...
raw_metadata_event_doc <- subset(raw_metadata_event, !is.na(DOC))

raw_metadata_event_doc$Collection_Date_YYYYMMDD <- gsub("-", "", raw_metadata_event_doc$Date)


raw_metadata_event_ids <- raw_metadata_event_doc %>%
  mutate(DOC_ID = paste("TMP",
                          Plot,
                          Grid,
                          Collection_Date_YYYYMMDD, 
                          Timepoint, 
                          sep = "_")) %>%
  rename(
    Collection_Start_Time_24hrs = Start_time, 
    Collection_End_Time_24hrs_meta = End_time, 
    Volume_mL = DOC, 
    Grid_meta = Grid,
    Plot_meta = Plot, 
    Collection_Date_YYYYMMDD_meta = Collection_Date_YYYYMMDD
  ) %>%
  select(Plot_meta, Grid_meta, Timepoint, Notes, Collection_Date_YYYYMMDD_meta, Collection_Start_Time_24hrs, Collection_End_Time_24hrs_meta, DOC_ID, Volume_mL)


```

## Pull in pre-treatment metadata
```{r rest of metadata, include=FALSE}
...
colnames(raw_metadata_pt) <- c("Plot_meta", "Grid_meta", "Depth_cm", "Volume_mL_20220613", "Volume_mL_20220615", "Bulk_mL_20220613",  "Bulk_mL_20220615", "Notes", "NA", "NA1")

##Convert to long format
raw_metadata_pt_long <- raw_metadata_pt %>%
  select("Plot_meta", "Grid_meta", "Depth_cm", "Volume_mL_20220613", "Volume_mL_20220615") %>%
  mutate(
  ...
  pivot_longer(
    ...
    names_to = "Collection_Date_YYYYMMDD_meta",
  ...
  mutate(
    Plot_meta = case_when(
      Plot_meta == "CONTROL" ~ "C",
      Plot_meta == "FRESHWATER" ~ "FW",
      Plot_meta == "SEAWATER" ~ "SW",
      TRUE ~ Plot_meta  # keep everything else unchanged
    )
  ) %>%
  mutate(
...
raw_metadata_pt_ids <- raw_metadata_pt_long %>%
  mutate(DOC_ID = paste("TMP",
                          Plot_meta,
                          Grid_meta,
                          Collection_Date_YYYYMMDD_meta,
                          sep = "_"))

##select only ones with volumes
raw_metadata_pt_ids <- subset(raw_metadata_pt_ids, Volume_mL > 0)

```


## Combine event, non-event, and pre-treatment metadata
```{r}

doc_metadata_nonevent$Collection_Start_Time_24hrs <- as.character(doc_metadata_nonevent$Collection_Start_Time_24hrs)
doc_metadata_nonevent$Collection_Date_YYYYMMDD_meta <- as.character(doc_metadata_nonevent$Collection_Date_YYYYMMDD_meta)

doc_metadata <- bind_rows(doc_metadata_nonevent, raw_metadata_event_ids, raw_metadata_pt_ids)
...
final_data <- merged_data %>%
  mutate(Run_notes = ifelse(Plot %in% no_meta_flags, "Sample did not have associated metadata after checking sample sheets", NA)) %>%
  mutate(
    Depth_cm = Depth, 
    Run_notes = Run_notes, 
    Field_notes = Notes, 
    ...
    Analysis_runtime = "NA", 
    Analyte = "DOC", 
    Collection_End_Time_24hrs  = format(strptime(Collection_End_Time_24hrs_meta, format="%H:%M:%S"), "%H%M"), 
    Collection_Time_24hrs = Collection_Time_24hrs_meta, 
  ) %>%
  rename(
    npoc_mgL = doc_mg_l, 
    npoc_uM = doc_uM, 
 ...
 select(
    ...
    Collection_Start_Time_24hrs, Collection_End_Time_24hrs, Collection_Time_24hrs, 
    EST_EDT, Volume_mL
