...
#anything that needs to be changed do this in the first chunk
  ...
  #file path and name for summary file 
    raw_file_name = c("tmp_doc_raw_data_2026/TMP_202606_1.txt", "tmp_doc_raw_data_2026/TMP_202606_2.txt", "tmp_doc_raw_data_2026/TMP_202606_3.txt" )
  #file path and name for the all peaks file 
    raw_allpeaks_name = c("tmp_doc_raw_data_2026/TMP_202606_1_allpeaks.txt", "tmp_doc_raw_data_2026/TMP_202606_2_allpeaks.txt", "tmp_doc_raw_data_2026/TMP_202606_3_allpeaks.txt" )
  #file path and name for processed data after QAQC
    processed_file_name = "tmp_doc_processed_data_2026/TMP_PW_DOC_Processed_202606.csv"
...
#Log path 
    Log_path = "tmp_doc_raw_data_2026/COMPASS_TMP_TOCTN_QAQClog_2026.csv"
...
directory= file.path(here() %>% dirname(), 'TEMPEST_Porewater/processing_scripts/DOC/tmp_doc_2026/tmp_doc_metadata_2026')
...
#one of the curve ran over midnight, 
#so I want to change the run date so they are grouped together on the plot: 
stds_N <- stds_N %>%
  mutate(run_date = if_else(
      standard_N_ppm == 3.00 & as.Date(run_date) == as.Date("2026-03-18"),
      as.POSIXct("2026-03-17", tz = "UTC"),
      run_date
    )
  )
...
colnames(IDs) <- c("Site_Code" ,"Plot", "Grid_Square", "Depth_cm", "Date", "Time_Point")
#head(IDs)

#rejoin them to the dataframe
dat_florplot <- cbind(IDs, dat_flagged)
dat_florplot$Plot <- as.factor(dat_florplot$Plot)
#head(dat_florplot)
...
dat_florplot <- dat_florplot %>% 
  mutate(Plot_name = case_when(Plot == 'SW' ~ "Saltwater", 
                               ...
                               Plot == 'EST' ~ "Estuary")) %>%
  mutate(
    id = paste(Grid_Square, Time_Point, sep = "_"))

#Plot data and change colors based on flags to check it: 
viz_c_plot <-  ggplot(data = dat_florplot, aes(x = id, y = npoc_raw, fill=Plot_name)) +
       geom_bar(stat = 'identity') + 
        ...
        scale_fill_manual(values = c("Control" = "springgreen2", "Freshwater" = "cyan2",
                                      "Saltwater" = "violetred2", "Estuary" = "darkred")) +
        theme_classic() + labs(x= " ", y="NPOC (mg/L)", title="Carbon by Plot") + 
        theme(legend.position="none") +
        theme(axis.text.x = element_text(angle = 90, hjust = 0.5))


viz_n_plot <-  ggplot(data = dat_florplot, aes(x = id, y = tdn_raw, fill=Plot_name)) +
       geom_bar(stat = 'identity') + 
        facet_grid(~Plot_name, scales="free_x") +
        #facet_grid(~factor(Plot_name, levels=c('Control', 'Freshwater', 'Saltwater'))) + 
        scale_fill_manual(values = c("Control" = "springgreen2", "Freshwater" = "cyan2",
...
```{r Visualize Event Data, echo=FALSE}

cat("Visualize Event Data")

#Separate PW and Source Waters 
dat_florplot_sources <- dat_florplot %>%
  filter(Grid_Square == "SCE") %>%
  mutate(
    Plot_name = factor(
      Plot_name,
      levels = c( "Freshwater",  "Saltwater", "Estuary") ))

#Plot data and change colors based on flags to check it: 
viz_c_plot_sce <-  ggplot(data = dat_florplot_sources, aes(x = id, y = npoc_raw, fill=Plot_name)) +
       geom_bar(stat = 'identity') + 
        facet_grid(~Plot_name, scales="free_x") + 
        #facet_grid(~factor(Plot_name, levels=c('Control', 'Freshwater', 'Saltwater'))) + 
        scale_fill_manual(values = c("Control" = "springgreen2", "Freshwater" = "cyan2",
                                      "Saltwater" = "violetred2", "Estuary" = "darkred")) +
        theme_bw() + labs(x= " ", y="NPOC (mg/L)", title="Source DOC by Plot") + 
        theme(legend.position="none") +
        theme(axis.text.x = element_text(angle = 90, hjust = 0.5))


viz_n_plot_sce <-  ggplot(data = dat_florplot_sources, aes(x = id, y = tdn_raw, fill=Plot_name)) +
       geom_bar(stat = 'identity') + 
        facet_grid(~Plot_name, scales="free_x") +
        #facet_grid(~factor(Plot_name, levels=c('Control', 'Freshwater', 'Saltwater'))) + 
        scale_fill_manual(values = c("Control" = "springgreen2", "Freshwater" = "cyan2",
...
#ggarrange(viz_c_plot, viz_n_plot, nrow=1, ncol=2)
print(viz_c_plot_sce)
print(viz_n_plot_sce)

#Runoff 
#Separate PW and Source Waters 
dat_florplot_ro <- dat_florplot %>%
  filter(Grid_Square == "RO") %>%
  mutate(
    Plot_name = factor(
      Plot_name,
...
#Plot data and change colors based on flags to check it: 
viz_c_plot_ro <-  ggplot(data = dat_florplot_ro, aes(x = id, y = npoc_raw, fill=Plot_name)) +
       geom_bar(stat = 'identity') + 
        facet_grid(~Plot_name, scales="free_x") + 
        #facet_grid(~factor(Plot_name, levels=c('Control', 'Freshwater', 'Saltwater'))) + 
        scale_fill_manual(values = c("Control" = "springgreen2", "Freshwater" = "cyan2",
                                      "Saltwater" = "violetred2", "Estuary" = "darkred")) +
        theme_bw() + labs(x= " ", y="NPOC (mg/L)", title="Runoff DOC by Plot") + 
        theme(legend.position="none") +
        theme(axis.text.x = element_text(angle = 90, hjust = 0.5))


viz_n_plot_ro <-  ggplot(data = dat_florplot_ro, aes(x = id, y = tdn_raw, fill=Plot_name)) +
       geom_bar(stat = 'identity') + 
        facet_grid(~Plot_name, scales="free_x") +
        #facet_grid(~factor(Plot_name, levels=c('Control', 'Freshwater', 'Saltwater'))) + 
        scale_fill_manual(values = c("Control" = "springgreen2", "Freshwater" = "cyan2",
...
#Separate PW and Source Waters 
dat_florplot_pw <- dat_florplot %>%
  filter(!Grid_Square %in% c("SCE", "RO", "MP")) %>%
  mutate(
    Plot_name = factor(
      Plot_name,
      levels = c( "Freshwater",  "Saltwater", "Control") ))

#Plot data and change colors based on flags to check it: 
viz_c_plot_pw <-  ggplot(data = dat_florplot_pw, aes(x = id, y = npoc_raw, fill=Plot_name)) +
       geom_bar(stat = 'identity') + 
        facet_grid(~Plot_name, scales="free_x") + 
        #facet_grid(~factor(Plot_name, levels=c('Control', 'Freshwater', 'Saltwater'))) + 
        scale_fill_manual(values = c("Control" = "springgreen2", "Freshwater" = "cyan2",
                                      "Saltwater" = "violetred2", "Estuary" = "darkred")) +
        theme_bw() + labs(x= " ", y="NPOC (mg/L)", title="Porewater DOC by Plot") + 
        theme(legend.position="none") +
        theme(axis.text.x = element_text(angle = 90, hjust = 0.5))


viz_n_plot_pw <-  ggplot(data = dat_florplot_pw, aes(x = id, y = tdn_raw, fill=Plot_name)) +
       geom_bar(stat = 'identity') + 
        facet_grid(~Plot_name, scales="free_x") +
        #facet_grid(~factor(Plot_name, levels=c('Control', 'Freshwater', 'Saltwater'))) + 
        scale_fill_manual(values = c("Control" = "springgreen2", "Freshwater" = "cyan2",
...
dat_florplot_pw <- dat_florplot_pw %>%
    mutate(
    Time_Point = if_else(Time_Point == "TMP", "Pre", Time_Point)) %>%
    mutate(
    Time_Point = factor(
      Time_Point,
      levels = c( "Pre",  "D1-PM", "D2-AM", "D2-PM", "D3-AM", "D3-PM", "D4-AM", "D4-PM", "D5-AM", "D5-PM") ))

viz_c_boxplot_pw <-  ggplot(data = dat_florplot_pw, aes(x = Time_Point, y = npoc_raw, fill=Plot_name)) +
       geom_boxplot() + 
        facet_grid(~Plot_name, scales="free_x") + 
        ylim(0,75) +
        #facet_grid(~factor(Plot_name, levels=c('Control', 'Freshwater', 'Saltwater'))) + 
...
viz_c_boxplot_pw1 <-  ggplot(data = dat_florplot_pw, aes(x = Time_Point, y = npoc_raw, fill=Plot_name)) +
       geom_boxplot() + 
        facet_grid(Depth_cm~Plot_name, scales="free_x") + 
        ylim(0,75) +
        #facet_grid(~factor(Plot_name, levels=c('Control', 'Freshwater', 'Saltwater'))) + 
        scale_fill_manual(values = c("Control" = "springgreen2", "Freshwater" = "cyan2",
                                      "Saltwater" = "violetred2", "Estuary" = "darkred")) +
        theme_bw() + labs(x= " ", y="NPOC (mg/L)", title="Porewater DOC by Plot & Depth") + 
        theme(legend.position="none") +
        theme(axis.text.x = element_text(angle = 90, hjust = 0.5))
...
sample_collected_metadata <- readxl::read_excel(file_path, skip=3, sheet="Porewater - Individual") %>%
  select(Project, Plot, Sample_ID, Vial_ID, Volume_mL, Evacuation_date_YYYMMDD, Collection_Date_YYYYMMDD, 
...
samples_collected <- sample_collected_metadata %>%
  ...
  mutate(plot = stringr::str_extract(Vial_ID, 'SW|FW|C'),
         ...
         depth = stringr::str_extract(Sample_ID, "15|30|50"), 
         date = stringr::str_extract(Vial_ID, "[0-9]{8}"),
         sample_name = paste0("TMP", "_", plot, "_", grid, "_", date, "_", depth, "cm" )) 

# Clean up the data to make sure it will match the metadata file 
dat_flagged1 <- dat_flagged %>%
  mutate(sample_name = str_replace(sample_name, "CTRL", "C")) %>%
  # Remove rows where sample_name ends with "_dup"
  filter(!str_detect(sample_name, "_dup$")) %>% 
  mutate(plot = stringr::str_extract(sample_name, 'SW|FW|C'),
         grid = stringr::str_extract(sample_name, "B4|C3|C6|D5|E3|F4|F6|H3|H6|I5") ,
         depth = stringr::str_extract(sample_name, "15cm|30cm|50cm"), 
         date = stringr::str_extract(sample_name, "[0-9]{8}"),
         sample_name = paste0("TMP", "_", plot, "_", grid, "_", date, "_", depth)) %>%
 select(-c(plot, grid, depth, date))
