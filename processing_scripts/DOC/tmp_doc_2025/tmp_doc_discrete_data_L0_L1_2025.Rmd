...
folder_2025 <- "~/GitHub/TEMPEST_Porewater/processing_scripts/DOC/tmp_doc_2025/tmp_doc_processed_data_2025/"

files_2025 <- list.files(path = folder_2025, pattern = "*.csv", full.names = TRUE)

all_2025 <- bind_rows(lapply(files_2025, 
                             function(file){
                               read_csv(file) %>%
...
View(all_2025)
```
#### need to add in something about checking notes / flags for obvious removals if necessary.
...
clean_2025 <- all_2025 %>%
  ...
  ## Note, talk to steph if we want to include the mM or not of DOC
  # Also discuss if we want to code in the "lysimeter empty" flag for samples without DOC concentrations 

View(clean_2025)
  
write_csv(clean_2025, paste0(folder_2025, "01_TMP_PW_NPOCTDN_2025_L1_updated.csv") ) #note this is currently set up so that it writes into the folder youre pulling data from so if you run script more than once it will pull in the old file.
