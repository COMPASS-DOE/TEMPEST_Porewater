---
title: "COMPASS TEMPEST Discrete DOC Data Workflow: 202502"
author: "January & February 2025"
date: "`r Sys.Date()`"
output: pdf_document
---
## Run Information
...
#anything that needs to be changed do this in the first chunk
  ...
  run_notes = "Standard curve for TN is not good, reanalyzed with a
  recently run standard curve to get values."
 
  #file path and name for summary file 
    raw_file_name = "tmp_doc_raw_data_2025/TMP_202502.txt" 
  #file path and name for the all peaks file 
    raw_allpeaks_name = "tmp_doc_raw_data_2025/TMP_202502_allpeaks.txt"
  #file path and name for processed data after QAQC
    processed_file_name = "tmp_doc_processed_data_2025/TMP_PW_DOC_Processed_202502.csv"
...
#Log path 
    Log_path = "tmp_doc_raw_data_2025/COMPASS_TMP_TOCTN_QAQClog_2025.csv"
...
directory= file.path(here() %>% dirname(), 'TEMPEST_Porewater/processing_scripts/DOC/tmp_doc_2025/tmp_doc_metadata_2025')
...
dups <- dat_raw
dups$sample_name <- gsub("check", "dup", dups$sample_name) ##Change "check" samples to dups
dups <- dups %>%  
...
colnames(IDs) <- c("Site_Code" ,"Plot", "Grid_Square",  "Date", "NA")
head(IDs)
...
dat_flagged <- dat_flagged %>%
  ...
  filter(!str_detect(sample_name, "_check$")) ##Remove check samples from final dataframe because these are basically dups
