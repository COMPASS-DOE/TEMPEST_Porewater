...
## Summarized data for Site and Grid 
...
##Metrics for each Grid for all years
summary_all_years_source <- data_plotting_source %>%
  group_by(plot_1) %>%
   reframe(
     min_npoc_mgL = min(npoc_mgL, na.rm=TRUE),
     max_npoc_mgL = max(npoc_mgL, na.rm=TRUE), 
...
kable(
  summary_by_year_source[, c("plot_1", "min_npoc_mgL",	"max_npoc_mgL",	"mean_npoc_mgL", "min_tdn_mgL",	"max_tdn_mgL",	"mean_tdn_mgL")],  
  format = "latex",
  booktabs = TRUE,
  caption = "Sourcewater Summary Statistics by Grid"
) %>%
  kable_styling(
    latex_options = c("hold_position", "striped", "scale_down"),
    font_size = 12
...
#write out a csv of the metadata associated with the data: 
write.csv(summary_all_years_source, "COMPASS_TEMPEST_SOURCE_DOC_Summarized_Years_Combined.csv")
