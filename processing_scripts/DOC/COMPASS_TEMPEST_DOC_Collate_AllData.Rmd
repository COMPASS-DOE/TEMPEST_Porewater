...
##Combine samples from before 2026 when new depths were added
raw_dat_pre26 <- bind_rows(dat22, dat23, dat24, dat25)

##Sourcewater samples should be NA for depth, all other samples have depths. 
raw_dat_pre26 <- raw_dat_pre26 %>% 
  mutate(Depth_cm = ifelse(str_detect(Sample_ID, "SOURCE"), NA, Depth_cm)) %>% 
  mutate(Depth_cm = gsub("cm", "", Depth_cm)) %>% 
  mutate(grid = ifelse(grid == "Source", "Sourcewater", grid)) %>% 
  mutate(grid = ifelse(str_detect(Sample_ID, "Rain"), "Rainwater", grid)) %>% 
  mutate(Sample_ID = gsub("_15CM", "", Sample_ID)) %>%
  mutate(Sample_ID = gsub("_15cm", "", Sample_ID)) %>%
  rowwise() %>%
  mutate(
    Sample_ID = if (is.na(Depth_cm)) {
      Sample_ID
    } else {
      sub("([0-9]{8})", paste0(Depth_cm, "_\\1"), Sample_ID)
    }
  ) %>%
  ungroup()


#read in 2026 DOC Data:
dat26 <- read.csv("tmp_doc_2026/COMPASS_TEMPEST_DOC_2026.csv", colClasses = "character")
# head(dat26)
colnames(dat26)

##combine with the rest of the data
raw_dat <- bind_rows(raw_dat_pre26, dat26)



##Convert Collection Date into Date format
raw_dat$Collection_Date_YYYYMMDD <- as.character(raw_dat$Collection_Date_YYYYMMDD)
raw_dat$Collection_Date_formatted <- as.Date(raw_dat$Collection_Date_YYYYMMDD, format = "%Y%m%d")
...
unique(all_dat$npoc_flag)
unique(all_dat$tdn_flag)

all_dat$npoc_flag <- gsub("≥", "greater than or equal to", all_dat$npoc_flag)
all_dat$tdn_flag <- gsub("≥", "greater than or equal to", all_dat$tdn_flag)
all_dat$tdn_flag <- gsub(">", "greater than", all_dat$tdn_flag)
all_dat$tdn_flag <- gsub("<", "less than", all_dat$tdn_flag)

##Remove any notes about vibes
all_dat <- all_dat %>% 
  mutate(Field_notes = ifelse(str_detect(Field_notes, "vibe|Vibe"), NA, Field_notes))
...
data_plotting <- raw_dat %>%
  filter(!str_detect(grid, "Sourcewater|Rainwater"))
...
viz_npoc_plot <- ggplot(data_plotting, aes(x = as.character(Year), y = npoc_mgL, fill = plot_1)) +
  ...
  facet_wrap( ~ plot_1, scales="free_x") +
...
####Figure for DOC Paper####
viz_npoc_plot_avgs <- ggplot(DOC_avg, aes(x = Collection_Date_formatted, y = mean_npoc, color = plot_1)) +
  geom_point(size = 2) + 
  geom_errorbar(aes(ymin = mean_npoc - sd_npoc, ymax = mean_npoc + sd_npoc), width = 3, linewidth = 1) + 
  geom_line(linewidth = 1) + 
  geom_vline(xintercept = as.Date("2023-06-07"), linetype = "dashed", color = "black", size = 1) + 
  geom_vline(xintercept = as.Date("2024-06-12"), linetype = "dashed", color = "black", size = 1) + 
  geom_vline(xintercept = as.Date("2024-06-13"), linetype = "dashed", color = "black", size = 1) + 
  # geom_vline(xintercept = as.Date("2026-06-08"), linetype = "dashed", color = "black", size = 1) + 
  # geom_vline(xintercept = as.Date("2026-06-09"), linetype = "dashed", color = "black", size = 1) + 
  # geom_vline(xintercept = as.Date("2026-06-10"), linetype = "dashed", color = "black", size = 1) + 
  # geom_vline(xintercept = as.Date("2026-06-11"), linetype = "dashed", color = "black", size = 1) + 
  facet_wrap( ~ plot_1, ncol = 1) +
  ...
  theme_classic() +
  labs(x = "Date", y = "NPOC (mg/L)") +
  theme(
    panel.spacing = unit(0.5, "lines"), 
    panel.border = element_rect(color = "black", fill = NA, linewidth = 1)) +
  theme(text = element_text(size = 30)) + 
  scale_x_date(
    breaks = as.Date(c("2022-06-01", "2023-06-01", "2024-06-01", "2025-06-01")),
    date_labels = "%B %Y", 
    date_minor_breaks = "1 month", 
    guide = guide_axis(minor.ticks = TRUE)
  ) + 
  theme(axis.ticks.x = element_line(linewidth = 1))+
  theme(axis.ticks.length.x = unit(0.45, "cm"))+ 
  theme(axis.minor.ticks.length.x = unit(0.15, "cm"))+ 
  theme(legend.position = "none") +
  theme(
    # Adds a 20pt margin to the top, right, bottom, and left edges
...
# Save as a PNG
png("COMPASS_SynopticCB_NPOC_timeline.png", width = 2000, height = 1200, res = 100)
print(viz_npoc_plot_avgs)  # For ggplot objects, use print()
dev.off()
...
viz_npoc_boxplot <- ggplot(data_plotting, aes(x = Collection_Date_YYYYMMDD,  y = npoc_mgL, fill = plot_1)) +
  geom_boxplot() + 
  facet_wrap( ~ plot_1, ncol = 1) +
...
viz_tdn_boxplot <- ggplot(data_plotting, aes(x = Collection_Date_YYYYMMDD,  y = tdn_mgL, fill = plot_1)) +
  geom_boxplot() + 
  facet_wrap( ~ plot_1, ncol = 1) +
  scale_x_discrete(drop = TRUE) +
  scale_fill_manual(values = c(
...
##Boxplot Summarize DOC by Date, Site, and Grid
viz_npoc_boxplot <- ggplot(data_plotting_source, aes(x = Collection_Date_YYYYMMDD,  y = npoc_mgL, fill = plot_1)) +
  geom_boxplot() + 
  facet_wrap( ~ plot_1, ncol = 1) +
  # scale_x_discrete(drop = TRUE) +
  scale_fill_manual(values = c(
...
viz_tdn_boxplot <- ggplot(data_plotting_source, aes(x = Collection_Date_YYYYMMDD,  y = tdn_mgL, fill = plot_1)) +
  geom_boxplot() + 
  facet_wrap( ~ plot_1, ncol = 1) +
  scale_x_discrete(drop = TRUE) +
  scale_fill_manual(values = c(
...
npoc <- ggplot(data_plotting, aes(x = grid,  y = npoc_mgL, fill = plot_1)) +
  geom_boxplot() + 
  ...
  theme_classic() +
  labs(x = "Grid ID", y = "NPOC (mg/L)", title = "TEMPEST Porewater: NPOC by Grid") +
  theme(
    panel.spacing = unit(0.5, "lines"), 
    panel.border = element_rect(color = "black", fill = NA, linewidth = 1)) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))+ 
...
viz_tdn_boxplot <- ggplot(data_plotting, aes(x = grid,  y = tdn_mgL, fill = plot_1)) +
  geom_boxplot() + 
  facet_wrap( ~ plot_1, scales="free_y", ncol = 1) +
  scale_x_discrete(drop = TRUE) +
  scale_fill_manual(values = c(
  ...
  theme_classic() +
  labs(x = "Grid ID", y = "TDN (mg/L)", title = "TEMPEST Porewater: TDN by Grid") +
  theme(
    panel.spacing = unit(0.5, "lines"), 
    panel.border = element_rect(color = "black", fill = NA, linewidth = 1)) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))+ 
