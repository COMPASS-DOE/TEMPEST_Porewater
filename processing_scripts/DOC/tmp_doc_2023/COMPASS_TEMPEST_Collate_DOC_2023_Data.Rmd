...
TMP_SW_F6_20230417, TMP_SW_D5_20230417, TMP_SW_E3_20230417, TMP_SW_H6_20230417, TMP_FW_D5_20230605, TMP_SW_H6_20230612, TMP_C_C3_20230628, TMP_SW_I5_20230807, TMP_C_H6_20230810, TMP_SW_C6_20230810, TMP_SW_H3_20230810, TMP_SW_E3_20230810, TMP_C_I5_20231005, TMP_SW_C6_20231211"
```
...
all_data$Time_of_day = ifelse(all_data$Time<1200, "AM", "PM")
all_data <- all_data %>% 
  mutate(Sample_ID = ifelse(Date == "20230607", paste(Site, Plot, Grid, Date, Time_of_day, sep = "_"), sample_name))
...
all_data_sw <- all_data_sw %>% 
  mutate(
    Sample_ID = paste(Site, Plot, Grid, Date, Event_Time, sep = "_")
  )
...
all_data3 <- all_data2 %>% 
  select("Sample_ID", "doc_mg_l", "tdn_mg_l", "doc_flag", "tdn_flag", "Time")
...
viz_doc_plot <- ggplot(data_plotting, aes(x = row_num, y = doc_mg_l, fill = Plot)) +
...
viz_tdn_plot <-  ggplot(data_plotting, aes(x = row_num, y = tdn_mg_l, fill = Plot)) +
  geom_bar(stat = "identity", position = position_dodge2(preserve = "single")) +
  facet_wrap(~ Plot, scales="free") +
  scale_fill_manual(values = c(
    "Control" = "springgreen2",
...
final_data <- merged_data %>%
 ...
 select(
    Project, plot, grid, Depth_cm, Analyte, Vial_ID, 
...
write.csv(final_data, "COMPASS_TEMPEST_DOC_2023.csv")
