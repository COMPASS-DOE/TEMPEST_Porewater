...
#anything that needs to be changed do this in the first chunk
  ...
  run_notes = "The TN went down during this run and there were no peaks for samples, 
    ...
    but were collected on 10-03. This is changed programmatically upon data read in."
  
  
  #file path and name for summary file 
    raw_file_name = "tmp_doc_raw_data_2024/TMP_202410.txt" 
  #file path and name for the all peaks file 
    raw_allpeaks_name = "tmp_doc_raw_data_2024/TMP_202410_allpeaks.txt"
  #file path and name for processed data after QAQC
    processed_file_name = "tmp_doc_processed_data_2024/TMP_PW_DOC_Processed_202410.csv"
...
#need to fix two samples that were named incorrectly: 
dat_raw <- dat_raw %>%
  mutate(sample_name = ifelse(sample_name == "TMP_20241007_SW_D5", 
                              "TMP_20241003_SW_D5", sample_name)) %>%
  mutate(sample_name = ifelse(sample_name == "TMP_20241007_SW_E3", 
                              "TMP_20241003_SW_E3", sample_name)) %>% 
  filter(sample_name != "TMP_20241003_CON_C6") # Remove sample with clumped peaks
...
C_stds_plot <- ggplot(stds_C, aes(x = standard_C_ppm, y = area)) +
  ...
  geom_text(
    data = r2_labels_c,
    aes(x = x_pos, y = y_pos, label = paste0("R^2 = ", r_squared)),
    inherit.aes = FALSE,
    hjust = 1, vjust = 1,
    size = 4
  ) +
...
colnames(IDs) <- c("Site_Code" ,"Date", "Plot", "Grid_Square")
head(IDs)
...
sample_collected_metadata <- readxl::read_excel(file_path, skip=3, sheet="Porewater - Individual") %>%
  select(Project, Plot, Vial_ID, Volume_mL, Evacuation_date_YYYYMMDD, Collection_Date_YYYYMMDD, 
...
dat_flagged <- dat_flagged %>%
  ...
  mutate(sample_name = str_replace(sample_name, "CON", "C")) %>%
  # Remove rows where sample_name ends with "_dup"
  filter(!str_detect(sample_name, "_dup$")) %>%
  separate(sample_name, into = c("part1", "date", "part2", "part3"), sep = "_") %>%
  unite("sample_name", part1, part2, part3, date, sep = "_")
...
#### ADDED CODE DUE TO TN ISSUES WITH THIS RUN ####
final_data <- final_data %>% 
  #select(Project, Plot, grid, sample_name, Vial_ID, date, ) %>%
  mutate(
    tdn_mgL = "NA", 
    tdn_uM = "NA",
    tdn_flag = "TN broken, no data"
  ) 
