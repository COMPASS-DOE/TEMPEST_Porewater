...
#anything that needs to be changed do this in the first chunk
  ...
  run_notes = " SW_F4 is SW_F6 changed in this code. Rain water samples also
    collected, these are output to a separate file in processed data"
  
  #file path and name for summary file 
    raw_file_name = "tmp_doc_raw_data_2024/TMP_202412.txt" 
  #file path and name for the all peaks file 
    raw_allpeaks_name = "tmp_doc_raw_data_2024/TMP_202412_allpeaks.txt"
  #file path and name for processed data after QAQC
    processed_file_name = "tmp_doc_processed_data_2024/TMP_PW_DOC_Processed_202412.csv"
    processed_rain_data = "tmp_doc_processed_data_2024/TMP_RAIN_DOC_Processed_202412.csv"
...
directory= file.path('tmp_doc_metadata_2024')
if(!dir.exists(directory)){
  stop("Directory does not exist, check directory path")
}

#file path for download of the porewater inventory
file_path = file.path(directory, paste0("porewaterinventory_", "2026-02-11", ".xlsx"))
...
#new addition just for this month 
#we have a sample that was labeled incorrectly by accident, but we realized it
#SW-F4 should be SW-F6, metadata confirm that we did not collect enough water from 
#F4 for there to be a DOC sample, but F6 got a full compliment and there was no DOC sample
#we have determined that F4 is therefore F6 and that is changed here 

dat_raw <- dat_raw %>%
  mutate(sample_name = ifelse(sample_name == "TMP_SW_F4_20241213", 
                              "TMP_SW_F6_20241213", sample_name))
...
colnames(IDs) <- c("Site_Code" , "Plot", "Grid_Square", "Date", "Extra")
head(IDs)
...
final_data <- merged_data %>% 
  ...
  filter(!str_detect(sample_name, "Well")) %>%  
  filter(!str_detect(sample_name, "Rain"))
...
## Export Processed Rain Data  
```{r, Export Processed Rain Data, echo=FALSE}

cat("Export Processed Rain Data")

#Prepare data to be exported - if there is anything else to add 
#Add any necessary identifiers to the samples  ### VERY IMPORTANT AND STANDARD FOR PROJECT ####
  #example read in sample IDs list and merge 
  #create required ID columns in R, etc. 
final_data1 <- dat_florplot %>% 
  filter(str_detect(sample_name, "Rain")) %>%
  #select(Project, Plot, grid, sample_name, Vial_ID, date, ) %>%
  mutate(
    Project = "COMPASS: TEMPEST",
    sample_type = "DOC",# new column with same value on every row
    #Experiment = "TEMPEST",
    #Depth_cm = 15,
    plot = Plot,
    Type = Grid_Square,
...
final_data1 <- final_data1 %>%
  mutate(
    npoc_uM = (((as.numeric(final_data1$npoc_raw))/Con1)/mw_c)*Con2, 
    tdn_uM = (((as.numeric(final_data1$tdn_raw))/Con1)/mw_n)*Con2
  )
...
final_data1 <- final_data1 %>%
  ...
  select(
    Project, plot, Type, sample_type, Vial_ID, date, 
    npoc_mgL, npoc_uM, npoc_flag, tdn_mgL, tdn_uM, tdn_flag, Analysis_runtime,
...
#will put final data in processed data folder 
  write.csv(final_data1, processed_rain_data)
