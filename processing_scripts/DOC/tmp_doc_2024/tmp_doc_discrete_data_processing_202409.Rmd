...
#anything that needs to be changed do this in the first chunk
  ...
  run_notes = "This run the samples names were not put in correctly, they are
      changed in the code to be the correct format"
  
  #file path and name for summary file 
    raw_file_name = "tmp_doc_raw_data_2024/TMP_202409.txt" 
  #file path and name for the all peaks file 
    raw_allpeaks_name = "tmp_doc_raw_data_2024/TMP_202409_allpeaks.txt"
  #file path and name for processed data after QAQC
    processed_file_name = "tmp_doc_processed_data_2024/TMP_PW_DOC_Processed_202409.csv"
...
#Flag that we 
  ...
  chk_flag = 0.20   #if the RSD (relative standard deviation) is over 1 among checks this is the std dev. / mean  
  chks_flag = 60    #this is the percent of checks we want to be within range (at least) or else we will flag the data 
  chk_conc_flag = 20 #this is the level cutoff for percent difference of check standards vs. the concentration they are meant to be 
...
#added code because sample names were input wrongly
dat_raw <- dat_raw %>%
  mutate(sample_name = sub("_15cm$", "_20240906", sample_name))
...
chks_raw <- raw_file_name %>% 
  map_df(read_data) %>% 
  filter(grepl("1ppmCN_Check", sample_name)) %>% # filter to TMP samples only
  bind_rows() 
...
## Assess Duplicates - NO Duplicates in this run
