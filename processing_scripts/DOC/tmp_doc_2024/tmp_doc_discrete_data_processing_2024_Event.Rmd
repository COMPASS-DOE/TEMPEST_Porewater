...
directory= ('tmp_doc_metadata_2024')
...
all_dat_source <- all_dat_source %>%
  ...
  mutate(Event_Time = case_when(
    Time_of_day == "Morning" ~ "AM",
    Time_of_day == "Midday" ~ "MID",
    Time_of_day == "Afternoon" ~ "PM"
  ))
...
#separate tidal samples into columns TMP_ESTUARY_LT_20240611, HT AM, LT PM
...
final_data1 <- merged_data %>% 
  filter(sample_ID != "SW_C3_DOC_20240612_T3")
...
final_data <- final_data %>%
  ...
  select(
    ...
    Collection_Start_Time_24hrs, Collection_End_Time_24hrs, Collection_Time_24hrs, EST_EDT, Volume_mL
