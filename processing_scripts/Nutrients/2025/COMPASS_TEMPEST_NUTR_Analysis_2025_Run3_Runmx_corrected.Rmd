...
#create IDs from what was collected for comparison later
raw_metadata_sw_combined <- raw_metadata_sw_combined %>%
  mutate(NUTR_ID = ifelse(
    Notes %ilike% "tide", ifelse( Notes %ilike% "high",
                          paste(Project,
                          Source, 
                          Zone,
                          Collection_Date,
...
Chks_diff <- All_chks %>% mutate(Diff = ifelse(
  Test == "Vanadium NOx",
  100 * abs(Conc - NOx_CCV) / NOx_CCV,
  ifelse(
    Test == "Ammonia 2",
    100 * abs(Conc - NH3_CCV) / NH3_CCV,
    ifelse(Test == "o-PHOS 0.3", 100 * abs(Conc - PO4_CCV) / PO4_CCV, -999) #Calculate Check Percent Difference
  )
...
samples_flagged1 <- samples %>% mutate(Dilution = ifelse(Dilution == 0, 1, Dilution )) %>%
  mutate(
    ...
    Sample_Name %ilike% "MD" ~ str_replace(Sample_Name, "MD", "MID"),
    Sample_Name %ilike% "AN" ~ str_replace(Sample_Name, "AN", "PM"),
    Sample_Name %ilike% "MN" ~ str_replace(Sample_Name, "MN", "AM"),
    TRUE ~ Sample_Name)
  ) %>% mutate(Sample_Name = case_when(
    Sample_Name %ilike% "Est" ~ str_replace(Sample_Name, "Est", "Estuary"),
    TRUE ~ Sample_Name
  )
               )
