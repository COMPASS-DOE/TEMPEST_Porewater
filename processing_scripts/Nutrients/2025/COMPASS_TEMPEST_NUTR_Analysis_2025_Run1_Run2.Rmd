...
IDs_df_a <- data.frame(do.call('rbind', strsplit(as.character(final_data$Sample_ID),'_',fixed=TRUE)))
if(ncol(IDs_df_a) == 4) { IDs_df <- cbind(IDs_df_a, new_column = c("NA")) } else {IDs_df <- IDs_df_a}
colnames(IDs_df) <- c("Site_Code" ,
                      "Plot", 
                      "Grid_Square",  
                      "Date" ,
                      "Extra"
...
dat_forplot <- dat_forplot %>% 
  mutate(Plot_name = case_when(Plot == 'SW' ~ "Saltwater", 
                               Plot == 'FW' ~ "Freshwater", 
                               Plot == 'C' ~ "Control")) %>%
  mutate(Short_Names = Grid_Square) %>% group_by(Short_Names) %>% mutate(Multiple = ifelse(length(Short_Names) > 1, "YES", "NO")) %>% ungroup() %>% mutate(Short_Names = ifelse(Multiple == "YES", paste0(Short_Names, "_", Collection_Date), Short_Names))
