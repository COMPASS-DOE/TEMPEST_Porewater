---
title: "COMPASS_TEMPEST_SEAL_Data_Analysis"
author: "Stephanie J. Wilson"
date: '2023-06-04'
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## Information
```{r}
#######################################################################################
####### COMPASS TEMPEST
####### Data Analysis Code: Porewater Nutrients 
####### Data from pretreatment sampling in May 2023
##############################################################################################


############################# Information ##################################
#Author: Stephanie J. Wilson
#Edited: 20230606

#Samples taken from Lysimeters TEMPEST
#Samples were filtered with 0.45 uM, kept on ice, frozen until analysis
#Field Protocol: 
#Samples Analyzed on a SEAL discrete auto analyzer 
#Lab Protocol: https://docs.google.com/document/d/1VaJT7Wb9AcdmM1tgsR_9ZtQ6kwcaoNmp/edit?usp=sharing&ouid=108994740386869376571&rtpof=true&sd=true
#NOx method = https://drive.google.com/file/d/1sicqBFnzVxmDd5I2_pu8s8pj7iNOAuhF/view?usp=sharing
#NH4 method = https://drive.google.com/file/d/1ENGemUEvm_rffZqv3lz9BjD0pAMX5nzu/view?usp=sharing
#PO4 method = https://drive.google.com/file/d/1m3gXDZnJoIo_QmyhvZG4HRgGShCzm9Wq/view?usp=sharing
#Units from SEAL = mg/L and converted to uMoles/L
```

## QAQC on Slopes
```{r}
library(ggplot2)
library(data.table)

#read in datafile with all the slopes 
qlog <- read.csv("Raw Data/SERC_SEAL_STDs_Log.csv")
head(qlog)

#pull out each method 
qNH3 <- qlog[qlog$Analysis %like% "NH3", ]
head(qNH3)

qPO4 <- qlog[qlog$Analysis %like% "PO4", ]
head(qPO4)

qNOx <- qlog[qlog$Analysis %like% "V-Nox", ]
head(qNOx)

########### NH3
#plot the slopes to make sure there are no crazy outliers 
slope1 <- ggplot(data=qNH3, aes(x=Date, y=Slope)) +
           #geom_line()+
            geom_point(aes(size=3)) + 
            theme_classic() + ylim(0, 5) + 
           theme(legend.position="none") + 
           ggtitle("NH3 Slopes")
  
slope1

#plot the intercepts to make sure there are no crazy outliers 
int1 <- ggplot(data=qNH3, aes(x=Date, y=Intercept)) +
  #geom_line()+
  geom_point(aes(size=3)) + 
  theme_classic() + ylim(-0.5,0.5) + 
  theme(legend.position="none")+ 
  ggtitle("NH3 Intercepts")

int1

#plot the R2s to make sure there are no crazy outliers 
Rsq1 <- ggplot(data=qNH3, aes(x=Date, y=R2)) +
  #geom_line()+
  geom_point(aes(size=3)) + 
  theme_classic() + ylim(0.75, 1.25) + 
  theme(legend.position="none")+ 
  ggtitle("NH3 R2s")

Rsq1

########### PO4
slope2 <- ggplot(data=qNH3, aes(x=Date, y=Slope)) +
  #geom_line()+
  geom_point(aes(size=3)) + 
  theme_classic() + ylim(0, 5) + 
  theme(legend.position="none")+ 
  ggtitle("PO4 Slopes")

slope2

#plot the intercepts to make sure there are no crazy outliers 
int2 <- ggplot(data=qNH3, aes(x=Date, y=Intercept)) +
  #geom_line()+
  geom_point(aes(size=3)) + 
  theme_classic() + ylim(-0.5,0.5) + 
  theme(legend.position="none")+ 
  ggtitle("PO4 Intercepts")

int2

#plot the R2s to make sure there are no crazy outliers 
Rsq2 <- ggplot(data=qNH3, aes(x=Date, y=R2)) +
  #geom_line()+
  geom_point(aes(size=3)) + 
  theme_classic() + ylim(0.75, 1.25) + 
  theme(legend.position="none")+ 
  ggtitle("PO4 R2s")

Rsq2

########### NOx
slope3 <- ggplot(data=qNH3, aes(x=Date, y=Slope)) +
  #geom_line()+
  geom_point(aes(size=3)) + 
  theme_classic() + ylim(0, 5) + 
  theme(legend.position="none")+ 
  ggtitle("NOx Slopes")

slope3

#plot the intercepts to make sure there are no crazy outliers 
int3 <- ggplot(data=qNH3, aes(x=Date, y=Intercept)) +
  #geom_line()+
  geom_point(aes(size=3)) + 
  theme_classic() + ylim(-0.5,0.5) + 
  theme(legend.position="none")+ 
  ggtitle("NOx Intercepts")

int3

#plot the R2s to make sure there are no crazy outliers 
Rsq3 <- ggplot(data=qNH3, aes(x=Date, y=R2)) +
  #geom_line()+
  geom_point(aes(size=3)) + 
  theme_classic() + ylim(0.75, 1.25) + 
  theme(legend.position="none")+ 
  ggtitle("NOx R2s")

Rsq3



```

## Code Set up 
```{r}


#packages:
library(ggplot2)
library(dplyr)
library(data.table)
library(matrixStats)
library(gridExtra)
library(ggpubr)
library(grid)

```

## Ammonia & Phosphate
```{r}

#read in data 
file1 <- read.csv("Raw Data/SEAL_COMPASS_Synoptic_NH3_PO4_April2023_TEMPEST_May2023_2.csv")
#Quick look at dataframe
head(file1)
#take out only the columns that we need 
dat1 <- file1[ ,c(1,4,6,7, 12, 13)]
# assigning new names to the columns of the data frame
colnames(dat1) <- c('Run_Info','Sample_Name','Conc', "Abs", "Units", "Test")
head(dat1)


#read in data 
file2 <- read.csv("Raw Data/SEAL_COMPASS_NH3_PO4_TEMPEST_20230606.csv")
#Quick look at dataframe
head(file2)
#take out only the columns that we need 
dat2 <- file2[ ,c(1,4,6,7, 12, 13)]
# assigning new names to the columns of the data frame
colnames(dat2) <- c('Run_Info','Sample_Name','Conc', "Abs", "Units", "Test")
head(dat2)


#read in data 
file3 <- read.csv("Raw Data/SEAL_COMPASS_NH3_PO4_TEMPEST_20230607a.csv")
#Quick look at dataframe
head(file3)
#take out only the columns that we need 
dat3 <- file3[ ,c(1,4,6,7, 12, 13)]
# assigning new names to the columns of the data frame
colnames(dat3) <- c('Run_Info','Sample_Name','Conc', "Abs", "Units", "Test")
head(dat3)
# fix same ID
dat3$Sample_Name <- gsub("TEMPEST_FW_SOURCE_20230607_0621", "TEMPEST_SW_SOURCE_20230607_0621", dat3$Sample_Name)


#read in data 
file4 <- read.csv("Raw Data/SEAL_COMPASS_NH3_PO4_April2023_TEMPEST_May2023_3.csv")
#Quick look at dataframe
head(file4)
#take out only the columns that we need 
dat4 <- file4[ ,c(1,4,6,7, 12, 13)]
# assigning new names to the columns of the data frame
colnames(dat4) <- c('Run_Info','Sample_Name','Conc', "Abs", "Units", "Test")
head(dat4)

#read in data 
file5 <- read.csv("Raw Data/SEAL_COMPASS_NH3_PO4_TEMPEST_20230607b.csv")
#Quick look at dataframe
head(file5)
#take out only the columns that we need 
dat5 <- file5[ ,c(1,4,6,7, 12, 13)]
# assigning new names to the columns of the data frame
colnames(dat5) <- c('Run_Info','Sample_Name','Conc', "Abs", "Units", "Test")
head(dat5)


alldat <- rbind(dat1, dat2, dat3, dat4, dat5)


##Fix sample naming
alldat <- alldat %>% 
  mutate(Sample_Name = gsub("TEMEPST", "TEMPEST", Sample_Name)) %>% 
  mutate(Sample_Name = gsub("202300606", "20230606", Sample_Name))


#Pull out standards 
stds <- alldat[alldat$Sample_Name %like% "Standard", ]
head(stds)


#Pull out samples 
alldat2 <- alldat[alldat$Sample_Name %like% "TEM", ]
#alldat2 <- rbind(alldat2, (alldat[alldat$Sample_Name %like% "GWI_", ]))
#alldat2 <- rbind(alldat2, (alldat[alldat$Sample_Name %like% "GCrew_", ]))
head(alldat2)

```

## NOx
```{r}

#read in data 
Nfile1 <- read.csv("Raw Data/SEAL_COMPASS_Synoptic_NOx_April2023_TEMPEST_May2023_2.csv")
#Quick look at dataframe
head(Nfile1)
#take out only the columns that we need 
Ndat1 <- Nfile1[ ,c(1,4,6,7, 12, 13)]
# assigning new names to the columns of the data frame
colnames(Ndat1) <- c('Run_Info','Sample_Name','Conc', "Abs", "Units", "Test")
head(Ndat1)


#read in data 
Nfile2 <- read.csv("Raw Data/SEAL_COMPASS_NOx_TEMPEST_20230606.csv")
#Quick look at dataframe
head(Nfile2)
#take out only the columns that we need 
Ndat2 <- Nfile2[ ,c(1,4,6,7, 12, 13)]
# assigning new names to the columns of the data frame
colnames(Ndat2) <- c('Run_Info','Sample_Name','Conc', "Abs", "Units", "Test")
head(Ndat2)


#read in data 
Nfile3 <- read.csv("Raw Data/SEAL_COMPASS_NOx_TEMPEST_20230607a.csv")
#Quick look at dataframe
head(Nfile3)
#take out only the columns that we need 
Ndat3 <- Nfile3[ ,c(1,4,6,7, 12, 13)]
# assigning new names to the columns of the data frame
colnames(Ndat3) <- c('Run_Info','Sample_Name','Conc', "Abs", "Units", "Test")
head(Ndat3)

#read in data 
Nfile4 <- read.csv("Raw Data/SEAL_COMPASS_NOx_April2023_TEMPEST_May2023_3.csv")
#Quick look at dataframe
head(Nfile4)
#take out only the columns that we need 
Ndat4 <- Nfile4[ ,c(1,4,6,7, 12, 13)]
# assigning new names to the columns of the data frame
colnames(Ndat4) <- c('Run_Info','Sample_Name','Conc', "Abs", "Units", "Test")
head(Ndat4)

#read in data 
Nfile5 <- read.csv("Raw Data/SEAL_COMPASS_NOx_TEMPEST_20230607b.csv")
#Quick look at dataframe
head(Nfile5)
#take out only the columns that we need 
Ndat5 <- Nfile5[ ,c(1,4,6,7, 12, 13)]
# assigning new names to the columns of the data frame
colnames(Ndat5) <- c('Run_Info','Sample_Name','Conc', "Abs", "Units", "Test")
head(Ndat5)

#read in data 
Nfile6 <- read.csv("Raw Data/SEAL_COMPASS_NOx_TEMPEST_20230608.csv")
#Quick look at dataframe
head(Nfile6)
#take out only the columns that we need 
Ndat6 <- Nfile6[ ,c(1,4,6,7, 12, 13)]
# assigning new names to the columns of the data frame
colnames(Ndat6) <- c('Run_Info','Sample_Name','Conc', "Abs", "Units", "Test")
head(Ndat6)

Nalldat <- rbind(Ndat1 , Ndat2, Ndat3 , Ndat4, Ndat5, Ndat6)

#Pull out standards 
Nstds <- Nalldat[Nalldat$Sample_Name %like% "Standard", ]
head(Nstds)


#Pull out samples 
Nalldat2 <- Nalldat[Nalldat$Sample_Name %like% "TEM", ]
#Nalldat2 <- rbind(Nalldat2, (Nalldat[Nalldat$Sample_Name %like% "GWI_", ]))
#Nalldat2 <- rbind(Nalldat2, (Nalldat[Nalldat$Sample_Name %like% "GCrew_", ]))
head(Nalldat2)

##Fix sample IDs
Nalldat2$Sample_Name <- gsub("TEMPEST_20230513_CON_C6", "TEMPEST_20230515_CON_C6", Nalldat2$Sample_Name)
Nalldat2$Sample_Name <- gsub("TEMPEST_20230513_CON_H6", "TEMPEST_20230515_CON_H6", Nalldat2$Sample_Name)
Nalldat2$Sample_Name <- gsub("TEMPEST_20230513_FW_C3", "TEMPEST_20230515_FW_C3", Nalldat2$Sample_Name)
Nalldat2$Sample_Name <- gsub("TEMPEST_20230513_FW_C6", "TEMPEST_20230515_FW_C6", Nalldat2$Sample_Name)
Nalldat2$Sample_Name <- gsub("TEMPEST_20230513_FW_D5", "TEMPEST_20230515_FW_D5", Nalldat2$Sample_Name)
Nalldat2$Sample_Name <- gsub("TEMPEST_20230513_FW_E3", "TEMPEST_20230515_FW_E3", Nalldat2$Sample_Name)
Nalldat2$Sample_Name <- gsub("TEMPEST_20230513_FW_F6", "TEMPEST_20230515_FW_F6", Nalldat2$Sample_Name)
Nalldat2$Sample_Name <- gsub("TEMPEST_20230513_FW_B4", "TEMPEST_20230515_FW_B4", Nalldat2$Sample_Name)

##Fix sample naming
Nalldat2 <- Nalldat2 %>% 
  mutate(Sample_Name = gsub("TEMEPST", "TEMPEST", Sample_Name))

```

## Constants 
```{r}

N_mw <- 14.0067   # molecular weight of N 

P_mw <- 30.973762  # molecular weight of P 

Con1 <- 1000       # conversion factor value

Con2 <- 1000000    # conversion factor value 

```

## Convert Data from mg/L to uM 
```{r}
head(alldat2)
head(Nalldat2)

#subset by test
NH4samples <- subset(alldat2, Test == "Ammonia 2")
head(NH4samples)

PO4samples <- subset(alldat2, Test == "o-PHOS 0.3")
head(PO4samples)

NOXsamples <- subset(Nalldat2, Test == "Vanadium NOx")
head(NOXsamples)


NH4samples$Conc_uM_raw <- (((as.numeric(NH4samples$Conc))/Con1)/N_mw)*Con2
head(NH4samples)

PO4samples$Conc_uM_raw <- (((as.numeric(PO4samples$Conc))/Con1)/N_mw)*Con2
head(PO4samples)

NOXsamples$Conc_uM_raw <- (((as.numeric(NOXsamples$Conc))/Con1)/N_mw)*Con2
head(NOXsamples)

#add step to make negative values equal to bd (below detection) and replace with zeros 
NOXsamples$Conc_uM <- ifelse(NOXsamples$Conc_uM_raw<0, 0, (NOXsamples$Conc_uM_raw) )
head(NOXsamples)

NH4samples$Conc_uM <- ifelse(NH4samples$Conc_uM_raw<0, 0, (NH4samples$Conc_uM_raw) )
head(NH4samples)

PO4samples$Conc_uM <- ifelse(PO4samples$Conc_uM_raw<0, 0, (PO4samples$Conc_uM_raw) )
head(PO4samples)
```

## Pull all data back together and add flags
```{r}

#pull out the columns we want from each dataframe 
NH4_pull <- NH4samples[ ,c(2,3,7) ]
head(NH4_pull)

PO4_pull <- PO4samples[ ,c(2,3,7) ]
head(PO4_pull)

NOX_pull <- NOXsamples[ ,c(2,3,8) ]
head(NOX_pull)


#Bring all this data back together: 
all_data <- merge(NH4_pull, PO4_pull, by="Sample_Name", all.x=TRUE, all.y=TRUE)
all_data <- merge(all_data, NOX_pull, by="Sample_Name", all.x=TRUE, all.y=TRUE)
head(all_data)

colnames(all_data) <- c("Sample_Name", "NH3_mgL", "NH3_uM", "PO4_mgL", "PO4_uM", "NOx_mgL", "NOx_uM")
head(all_data)


#add in an if then statement that tells us if they are within the range of the test - check this after doing this line
all_data$NH3_range <- ifelse(all_data$NH3_mgL<0.02, "bdl",  ifelse(all_data$NH3_mgL>2, "adl", "Within_Range")) 
all_data$PO4_range <- ifelse(all_data$PO4_mgL<0.003, "bdl",  ifelse(all_data$PO4_mgL>3, "adl", "Within_Range")) 
all_data$NOx_range <- ifelse(all_data$NOx_mgL<0.025, "bdl",  ifelse(all_data$NOx_mgL>1, "adl", "Within_Range"))
head(all_data)

##Fix sample IDs
all_data$Sample_Name <- gsub("TEMPEST_FW_D5_202300606", "TEMPEST_FW_D5_20230606", all_data$Sample_Name)


```

## Take an initial look at concentrations 
```{r}
#plot data to get a sense of any outliers 
NH3look <- ggplot(data=all_data, aes(x=Sample_Name, y=NH3_uM)) +
  geom_bar(stat="identity") + 
  theme_classic() + ylim(-10, 200) + 
  theme(legend.position="none") + 
  ggtitle("Sample NH3 Concentrations")
NH3look

PO4look <- ggplot(data=all_data, aes(x=Sample_Name, y=PO4_uM)) +
  geom_bar(stat="identity") + 
  theme_classic() + ylim(-2, 50) + 
  theme(legend.position="none") + 
  ggtitle("Sample PO4 Concentrations")
PO4look

NOXlook <- ggplot(data=all_data, aes(x=Sample_Name, y=NOx_uM)) +
  geom_bar(stat="identity") + 
  theme_classic() + ylim(-2, 2) + 
  theme(legend.position="none") + 
  ggtitle("Sample NOx Concentrations")
NOXlook


```

## Pull out data you need, make IDs 
```{r}

head(all_data)

out <- all_data #[ ,c(1,3,5,7,8,9,10)]
head(out)

##0515 and 0608 data entered differently
# TEMPEST_20230515_FW_I5 - should be TEMPEST_FW_C3_20230607_0800
out_weird <- out %>%
  filter(str_detect(Sample_Name, "20230515|20230608|20230518"))
#pull the sample ID and separate it by the underscores 
IDs <- data.frame(do.call('rbind', strsplit(as.character(out_weird$Sample_Name),'_',fixed=TRUE)))
colnames(IDs) <- c("Project" , "Date","Plot", "Grid")
#rejoin them to the dataframe
alldat_weird <- cbind(IDs, out_weird)
alldat_weird$Time <- NA

out_normal <- out %>%
  filter(!str_detect(Sample_Name, "20230515"))%>%
  filter(!str_detect(Sample_Name, "20230608"))%>%
  filter(!str_detect(Sample_Name, "20230518"))
#for steph <- pull out identifiers of the sample names 
#pull the sample ID and separate it by the underscores 
IDs <- data.frame(do.call('rbind', strsplit(as.character(out_normal$Sample_Name),'_',fixed=TRUE)))
colnames(IDs) <- c("Project" , "Plot", "Grid", "Date", "Time")
#IDs$Month <- ifelse(IDs$Date == 202210,"October","November")
#IDs$Month <- "September"
head(IDs)

#rejoin them to the dataframe
alldat <- cbind(IDs, out_normal)
alldat$Time <- gsub("TEMPEST", NA, alldat$Time)
alldat$Time <- gsub("TEMEPST", NA, alldat$Time)


alldat_final <- rbind(alldat, alldat_weird)

##Remove duplicate sample
alldat_final <- alldat_final %>% 
  filter(Sample_Name != "TEMPEST_20230515_CON_C6")

```

## Export final data with flags 
```{r}

#Export Data
write.csv(alldat_final, file="Processed Data/COMPASS_TEMPEST_SEAL_NUTR_2023a.csv")

```


```{r}
# # Install the package if you haven't already
# # install.packages("arsenal")
# library(arsenal)
# 
# # Create example data frames
# df1 <- read.csv("Processed Data/COMPASS_TEMPEST_SEAL_NUTR_2023.csv")
# df2 <- read.csv("Processed Data/COMPASS_TEMPEST_SEAL_NUTR_2023a.csv")
# 
# # Compare the data frames
# comparison_result <- comparedf(df1, df2)
# 
# # View a summary of the differences
# summary(comparison_result)
# 
# # Install the package if you haven't already
# install.packages("daff")
# library(daff)
# 
# # Create example data frames
# df1 <- read.csv("Processed Data/COMPASS_TEMPEST_SEAL_NUTR_2023.csv")
# df2 <- read.csv("Processed Data/COMPASS_TEMPEST_SEAL_NUTR_2023a.csv")
# 
# # Generate the difference object
# diff_df <- diff_data(df1, df2)
# 
# # Print the differences
# diff_df
# 
# # Or view a visual HTML report in your browser
# render_diff(diff_df)

##Use 2023a data

```


