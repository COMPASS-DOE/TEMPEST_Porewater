...
df_all <- df1 %>% mutate(Run_Number = as.numeric(Run_Number),
         Run_Date = format(as.POSIXct(Run_Time,format='%m/%d/%Y %H:%M'),format='%m/%d/%Y')) %>% mutate(Sample_Name = ifelse(Sample_Name %ilike% "TMP", gsub("TMP", "TEMPEST", Sample_Name),Sample_Name ))
