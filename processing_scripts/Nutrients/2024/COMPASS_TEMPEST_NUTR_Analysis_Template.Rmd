...
df_all <- df1 %>% mutate(Run_Number = as.numeric(Run_Number),
         Run_Date = format(as.POSIXct(Run_Time,format='%m/%d/%Y %H:%M'),format='%m/%d/%Y')) %>% mutate(Sample_Name = ifelse(Sample_Name %ilike% "TEMPEST", gsub("TEMPEST", "TMP", Sample_Name),Sample_Name ))
...
R2_Flags <- R2_Check %>% dplyr::select(Test, Run_Date, R2_Flag, R2) %>% arrange(Test, Run_Date) #make df for display 
...
Red_Eff_disp <- Red_Eff_2 %>% mutate(Red_Eff_Flag = ifelse(Mean_RedEff >= 95,
       ...
       "Mean NOx Reduction Efficiency <95% - REASSESS")) %>% dplyr::select(Test, Run_Date, Red_Eff_Flag) %>% distinct() %>% arrange(Test, Run_Date)
