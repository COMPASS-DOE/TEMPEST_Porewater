...
Red_Eff_disp <- Red_Eff_2 %>% mutate(Red_Eff_Flag = ifelse(Mean_RedEff >= 95,
       ...
       "Mean NOx Reduction Efficiency <95% - REASSESS")) %>% dplyr::select(Test, Run_Date, Red_Eff_Flag) %>% distinct()
...
#Plot Duplicates
Dup_Plot <-  ggplot(Dup_plot_df, aes(x = Short_Names, y = Duplicate_CV, fill =  Dup_Flag, color = Within_Range)) +
  geom_bar(stat = 'identity',
...
Spike_Plot <-  ggplot(Spike_plot_df, aes(x = Short_Names, y = Spike_CV, fill =  Spike_Flag)) +
  geom_bar(stat = 'identity',
    position = position_dodge2(preserve = "single"), linewidth = 0.75) +
