...
author: "2024 Samples Run 3 and 4"
date: "`r Sys.Date()`"
output: 
  pdf_document:
    toc: true
...
cat("Run Information: Brenden ") #lets you know what section you're in 
...
#identify the files you want to read in 
  ...
  NH3_PO4_files<- c("Raw Data/20260416_Tempest2024_NP_Run3.csv", "Raw Data/20260417_Tempest2024_NP_Run3rr_Run4.csv")
  NOx_files    <- c("Raw Data/20260416_Tempest2024_VNOx_Run3.csv","Raw Data/20260417_Tempest2024_VNOx_Run4.csv")
...
# Define the file path for QAQC log file - NO Need to change just check year 
  ...
  final_path <- "Processed Data/TEMPEST_PW_Processed_2024_Run3_4.csv"

#record any notes about the run or anything other info here: 
  run_notes <- "Run 3 needed samples reran because volume was low for NH4 and PO4, 0416 end of run had low volume for PE_Chk" 
...
raw_metadata_lys_combined <- raw_metadata_lys_combined %>%
  dplyr::select("Sample_ID", "Project", "Zone", "Source" ,"Grid", "Depth", "Analyte", "Collection_Date", "Evacuation_date_YYYMMDD", "Collection_Start_Time_24hrs", "Collection_End_Time_24hrs", "Event_Time", "Time_of_day", "Volume_mL", "Notes")
...
raw_metadata_sw_combined <- raw_metadata_sw_combined %>%
  rename("Notes" = "Notes:") %>%
  dplyr::select("Sample_ID", "Project", "Zone", "Source" ,"Grid", "Depth", "Analyte", "Collection_Date", "Deployment_Date_YYYYMMDD", "Deployment_Time_24hrs", "Collection_Time_24hrs", "Event_Time", "Time_of_day", "Volume_mL", "Notes")
...
Dup_plot_df <- Dup_Joined %>% filter(!Sample_Name %ilike% "chk") %>% filter(!Sample_Name %ilike% "Check") %>% group_by(Date_Num) %>% mutate(Duplicate_Diff = 100*abs(Dup_Conc - Conc)/mean(c(Dup_Conc, Conc))) %>% #Calculate cv of duplicates
...
df_all_clean <- samples_flagged_selected %>% 
  dplyr::select(Sample_Name, Test, Conc, Conc_uM, Conc_flag, Run_Date) %>% 
  group_by(Sample_Name) %>% 
  mutate(Count = length(Sample_Name), Run_Date_1 = as.Date(Run_Date, "%m/%d/%Y")) %>% 
  mutate(Keep = ifelse(Count <= 1, "Keep", ifelse(Run_Date_1 == max(Run_Date_1), "Keep", "RMV"))) %>% 
  filter(Keep == "Keep") %>% 
  dplyr::select(Sample_Name, Test, Conc, Conc_uM, Conc_flag, Run_Date) %>%
  left_join(nutr_metadata, by = c("Sample_Name" = "NUTR_ID")) %>% 
  left_join(Flags_Fin, by = join_by(Test, Run_Date))
...
final_data <- final_data_labeled %>%
  mutate(
    ...
    Sample_Name = ifelse(!is.na(Zone), paste(Sample_Name, "15cm", sep = "_"), Sample_Name)
    ) %>%
    rename(
    ...
    Deployment_Date = Deployment_Date_YYYYMMDD, 
    Plot = Zone
    # add more rename pairs as needed
  ) %>%
  dplyr::select(Sample_ID, Site, Plot, Grid, Depth_cm, Event_Time, Time_of_day, 
         Source, Volume_mL, Collection_Date, Evacuation_Date, 
         Collection_Start_Time_24hrs, Collection_End_Time_24hrs, 
...
dat_forplot <- final_data %>% 
  mutate(Plot_name = case_when(Plot == 'SW' ~ "Saltwater", 
                               Plot == 'FW' ~ "Freshwater", 
                               Plot == 'C' ~ "Control")) %>%
  mutate(Short_Names = str_extract(Sample_ID, "(?<=_).*(?=_)")) %>% 
  group_by(Short_Names) %>% 
  mutate(Multiple = ifelse(length(Short_Names) > 1, "YES", "NO")) %>% 
  ungroup() %>% 
  mutate(Short_Names = ifelse(Multiple == "YES", paste0(Short_Names, "_", Collection_Date), Short_Names)) %>% 
  mutate(Grid = ifelse(Grid == "RainWater", "RW", Grid)) %>% 
  filter(!is.na(Grid))

#Plot data and change colors based on flags to check it: 
viz_NH3_plot <-  ggplot(data = dat_forplot, aes(x = Grid, y = NH3_Conc_uM, fill=Plot_name)) +
      geom_bar(stat = 'identity') + 
...
viz_PO4_plot <-  ggplot(data = dat_forplot, aes(x = Grid, y = PO4_Conc_uM, fill=Plot_name)) +
       geom_bar(stat = 'identity') + 
        facet_grid(.~Plot_name, scales="free_x") + 
        #facet_grid(~factor(Plot_name, levels=c('Control', 'Freshwater', 'Saltwater'))) + 
        scale_fill_manual(values = c("Control" = "springgreen2", "Freshwater" = "cyan2",
...
viz_NOx_plot <-  ggplot(data = dat_forplot, aes(x = Grid, y = NOx_Conc_uM, fill=Plot_name)) +
       geom_bar(stat = 'identity') + 
        facet_grid(.~Plot_name, scales="free_x") + 
        #facet_grid(~factor(Plot_name, levels=c('Control', 'Freshwater', 'Saltwater'))) + 
        scale_fill_manual(values = c("Control" = "springgreen2", "Freshwater" = "cyan2",
