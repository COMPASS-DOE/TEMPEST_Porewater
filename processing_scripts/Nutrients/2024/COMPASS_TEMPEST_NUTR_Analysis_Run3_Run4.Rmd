...
df_all_clean <- samples_flagged_selected %>% dplyr::select(Sample_Name, Test, Conc, Conc_uM, Conc_flag, Run_Date) %>% group_by(Sample_Name) %>% mutate(Count = length(Sample_Name), Run_Date_1 = as.Date(Run_Date, "%m/%d/%Y")) %>% mutate(Keep = ifelse(Count <= 1, "Keep", ifelse(Run_Date_1 == max(Run_Date_1), "Keep", "RMV"))) %>% filter(Keep == "Keep") %>% dplyr::select(Sample_Name, Test, Conc, Conc_uM, Conc_flag, Run_Date) %>%
...
dat_forplot <- dat_forplot %>% 
  mutate(Plot_name = case_when(Plot == 'SW' ~ "Saltwater", 
                               Plot == 'FW' ~ "Freshwater", 
                               Plot == 'C' ~ "Control")) %>%
  mutate(Short_Names = str_extract(Sample_ID, "(?<=_).*(?=_)")) %>% group_by(Short_Names) %>% mutate(Multiple = ifelse(length(Short_Names) > 1, "YES", "NO")) %>% ungroup() %>% mutate(Short_Names = ifelse(Multiple == "YES", paste0(Short_Names, "_", Collection_Date), Short_Names)) %>% mutate(Grid_Square = ifelse(Grid_Square == "RainWater", "RW", Grid_Square))
