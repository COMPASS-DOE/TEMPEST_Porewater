...
Slope_raw_list <- map(
 ...
 #check R2 and flag
 Slope_Std_Crvs <- Slope_raw %>% dplyr::select(Run_Date, R2, Slope, Intercept, Quadratic, Test) %>% distinct() %>% mutate(R2_Flag = ifelse(R2 >= r2_cutoff, "R2_Pass", "R2_Fail"))
 ...
 Curves_Plot <- ggplot(For_crv_plot, aes(Conc, Absorbance, color = Run_Date)) + geom_point(size = 4) +
  geom_smooth(data = subset(For_crv_plot, Test != "Vanadium NOx"), method = "lm", formula = "y~x",se = FALSE) +
  geom_smooth(data = subset(For_crv_plot, Test == "Vanadium NOx"), method = "lm", formula = "y~x + I(x^2)",se = FALSE)+ facet_grid(.~Test, scales = "free") + labs(title = "Standard Curves") + theme_classic()
 
Curves_Plot
 

R2_Flags <- R2_Check %>% dplyr::select(Test, Run_Date, R2_Flag, R2)

R2_Flags_join <- R2_Flags %>% dplyr::select(Test, Run_Date, R2_Flag)

knitr::kable(R2_Flags)
...
#log <- tibble(Test = c(), Intercept= c(), Slope= c(), Quadratic= c(), Run_Date= c(), User= c())

old_log <- read.csv(file_path) %>% mutate(Run_Date = as.Date(as.character(Run_Date), tryFormats = c("%m/%d/%Y", "%Y-%m-%d")))
...
add_log <- R2_Check %>% dplyr::select(Test, Run_Date, R2, Slope, Intercept, Quadratic) %>%
  mutate(Test = case_when(
    ...
    Test == "o-PHOS 0.3" ~ "PO4"
  )) %>% mutate(Run_Date = as.Date(as.character(Run_Date), format = "%m/%d/%Y")) %>% mutate(User = user)


new_log <- rbind(old_log, add_log) %>% mutate(across(where(is.numeric), \(x) round(x, digits = 4))) %>% distinct()

write.csv(new_log, "Raw Data/SEAL_TEMPEST_QAQC_Log_2024.csv", row.names = FALSE)
...
df_all_cor <- df_all %>% na.omit() %>% group_by(Run_Number, Run_Date, Sample_Name, Test) %>% mutate(Keep = ifelse(Dilution == max(Dilution), "Keep", "Rmv")) %>% filter(Keep == "Keep") %>% ungroup() %>% mutate(Pair_ID = ifelse(
...
) %>% group_by(Run_Number, Run_Date, Sample_Name, Test) %>% mutate(Duplicated = ifelse(length(Sample_Name) == 1, "No Duplicates", "Yes Duplicates")) %>% filter(Duplicated == "Yes Duplicates") %>% ungroup()

df_dup_disp <- df_dup %>% dplyr::select(Sample_Name) %>% distinct()

df_dup_dil_samp <- df_dup %>% group_by(Run_Number, Run_Date, Sample_Name, Test) %>% mutate(Diluted = ifelse(length(unique(Dilution)) == 1, "No Dilution", "Yes Diluted")) %>% ungroup()

df_dup_dil_disp <- df_dup_dil_samp %>% filter(Diluted == "Yes Diluted") %>% dplyr::select(Sample_Name) %>% distinct()

df_dup_ndil_disp <- df_dup_dil_samp %>% filter(Diluted == "No Dilution") %>% dplyr::select(Sample_Name) %>% distinct()

#Which Samples were ran multiple times
ifelse(
...
NOx_peChk <- df_all_cor %>%
  ...
  mutate(
    ...
    NOx_pe_flag = ifelse(abs(pct_diff_Chk) < 25, "YES, Pass", "NO, rerun")) %>% dplyr::select(Pair_ID, Mean_Chk_Conc, Run_Date, pct_diff_Chk, Test, NOx_pe_flag) %>% distinct()
...
NH3_peChk <- df_all_cor %>%
  ...
  mutate(
    ...
    NH3_pe_flag = ifelse(abs(pct_diff_Chk) < 25, "YES, Pass", "NO, rerun")) %>% dplyr::select(Pair_ID, Mean_Chk_Conc, Run_Date, pct_diff_Chk, Test, NH3_pe_flag) %>% distinct()
...
PO4_peChk <- df_all_cor %>%
  ...
  mutate(
    ...
    PO4_pe_flag = ifelse(abs(pct_diff_Chk) < 25, "YES, Pass", "NO, rerun")) %>% dplyr::select(Pair_ID, Mean_Chk_Conc, Run_Date, pct_diff_Chk, Test, PO4_pe_flag) %>% distinct()
...
PE_Flag_join <- PE_Flag_disp %>% dplyr::select(Test, Run_Date, PE_Flag) %>% mutate(PE_Flag = ifelse(PE_Flag %ilike% "YES", "PE Check PASS", "PE Check FAIL"))
...
Red_Eff_disp <- Red_Eff_2 %>% mutate(Red_Eff_Flag = ifelse(Mean_RedEff >= 95,
       ...
       "Mean NOx Reduction Efficiency <95% - REASSESS")) %>% dplyr::select(Test, Run_Date, Red_Eff_Flag)
...
All_chks <- df_all %>% filter(str_detect(Sample_Name, "CCV") | str_detect(Sample_Name, "1mg/L ammonia") | str_detect(Sample_Name, "0.15 mg/L")) %>% group_by(Test) %>% mutate(rep = row_number(), RSV = sd(Conc)/mean(Conc), RSV_Flag = ifelse(RSV >= chk_flag, "RSV TOO HIGH - REASSESS", "RSV WITHIN RANGE - PROCEED"))  
...
Chks_diff <- All_chks %>% mutate(Diff = ifelse(Test == "Vanadium NOx", 100*(Conc - NOx_CCV)/NOx_CCV, ifelse(Test == "Ammonia 2", 100*(Conc - NH3_CCV)/NH3_CCV, ifelse(Test == "o-PHOS 0.3", 100*(Conc - PO4_CCV)/PO4_CCV, -999)))) %>% mutate(Diff_Flag = ifelse(Diff <= chk_conc_flag, "YES", "NO, rerun"))
...
samples <- df_all %>% 
  ...
  distinct() %>% 
  mutate(Quant = ifelse(Test == "Vanadium NOx", ifelse(Quant <= NOx_dl, 0.0125, Quant), Quant)) %>% ungroup()

blks_all <- left_join(blks_df, samples, by = join_by(Test, Run_Date))



blks_flagged <- blks_all %>% mutate(BLK_Flag = ifelse(Conc <= Quant, "YES", "NO, rerun")) %>% group_by(Test, Run_Date) %>% mutate(BLKs_Percent = 100*length(which(BLK_Flag == "YES"))/(length(BLK_Flag)) ) %>% mutate(BLK_Pct_Flag = ifelse(BLKs_Percent >= 60, ">60% of Blanks Pass - PROCEED", "<60% of Blanks Pass - REASSESS"))
...
Dup_plot_df <- Dup_Joined %>% filter(!Sample_Name %ilike% "chk") %>% group_by(Date_Num) %>% mutate(Duplicate_CV = cv(c(Dup_Conc, Conc))) %>% mutate(Dup_Flag = ifelse(Duplicate_CV >= 10, "NO, rerun", "YES")) %>% ungroup() %>% group_by(Sample_Name) %>% ungroup() %>% group_by(Sample_Name) %>% mutate(Num_Runs = length(Sample_Name), Max_Dil = max(Dilution), Rerun = ifelse(Num_Runs != 1 , ifelse(Dilution == Max_Dil, "KEEP", "REMOVE"), "KEEP")) %>% filter(Rerun == "KEEP")
...
Flags_Bad <- Flags_joined %>% mutate(across(!1:2, \(x) ifelse(x %ilike% "PROCEED" | x %ilike% "PASS", "", x))) %>% distinct()

Flags_Fin <- Flags_Bad %>% mutate(QAQC_Flag = paste0(R2_Flag, PE_Flag, Red_Eff_Flag, CHK_Flag, BLK_Pct_Flag, Dup_Flags, Spike_Flags, Matrix_Flags)) %>% dplyr::select(Test, Run_Date, Flags)
```
...
df_all_clean <- samples_flagged_selected %>%
  left_join(nutr_metadata, by = c("Sample_Name" = "NUTR_ID")) %>% dplyr::select(Sample_Name, Test, Conc, Conc_uM, Conc_flag, Run_Date) %>% left_join(Flags_Fin, by = join_by(Test, Run_Date))
...
#   dplyr::select(NUTR_ID, Month, Day, Depth_cm, Lysimeter, Time..24hr., Time.Zone_EDT.EST,  Field.Notes) 
# 
# #merge metadata with sample run data 
...
final_data <- final_data_labeled %>%
  ...
  select(Sample_ID, Site, Zone, Grid, Depth_cm, Event_Time, Time_of_day, 
