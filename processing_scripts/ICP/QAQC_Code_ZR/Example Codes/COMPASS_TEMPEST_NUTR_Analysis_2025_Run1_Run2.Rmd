...
#identify the files you want to read in 
  ...
  NH3_PO4_files<- c("Raw Data/20260421_Tempest2025_NP_Run1.csv", "Raw Data/20260422_Tempest2025_NP_Run2.csv")
  NOx_files    <- c("Raw Data/20260421_Tempest2025_VNOx_Run1.csv","Raw Data/20260422_Tempest2025_VNOx_Run2.csv")

# Define the file path for QAQC log file - NO Need to change just check year 
  file_path <- "Raw Data/SEAL_TEMPEST_QAQC_Log_2025.csv"
  final_path <- "Processed Data/TEMPEST_Nutrients_Processed_2025_Run1_Run2.csv"

#record any notes about the run or anything other info here: 
  run_notes <- "TMP_FW_I5_20250406 sample missing from IDs,
  0421 NH4 PE Check does not pass (~30% diff) but CCVs do,
  slope and intercept look normal, 
  and PE check was fine the next day" 
...
dat_forplot <- final_data %>% 
  mutate(Plot_name = case_when(Plot == 'SW' ~ "Saltwater", 
                               Plot == 'FW' ~ "Freshwater", 
                               Plot == 'C' ~ "Control"), 
         Short_Names = ifelse(Plot == "C", substr(Sample_ID, 5, 17), substr(Sample_ID, 5, 18)))

#Plot data and change colors based on flags to check it: 
viz_NH3_plot <-  ggplot(data = dat_forplot, aes(x = Short_Names, y = NH3_Conc_uM, fill=Plot_name)) +
      geom_bar(stat = 'identity') + 
...
viz_PO4_plot <-  ggplot(data = dat_forplot, aes(x = Short_Names, y = PO4_Conc_uM, fill=Plot_name)) +
       geom_bar(stat = 'identity') + 
        facet_grid(.~Plot_name, scales="free_x") + 
        #facet_grid(~factor(Plot_name, levels=c('Control', 'Freshwater', 'Saltwater'))) + 
        scale_fill_manual(values = c("Control" = "springgreen2", "Freshwater" = "cyan2",
...
viz_NOx_plot <-  ggplot(data = dat_forplot, aes(x = Short_Names, y = NOx_Conc_uM, fill=Plot_name)) +
       geom_bar(stat = 'identity') + 
        facet_grid(.~Plot_name, scales="free_x") + 
        #facet_grid(~factor(Plot_name, levels=c('Control', 'Freshwater', 'Saltwater'))) + 
        scale_fill_manual(values = c("Control" = "springgreen2", "Freshwater" = "cyan2",
