...
###### Run information - PLEASE CHANGE
  Sample_Year = "2026"  
  Date_Run = "2026-09-25"  #Date that instrument was run
  Run_by = "Zoe Read"  #Instrument user 
  Script_run_by = "Zoe Read" #Code user 
  run_notes = "Run looks good other than one bad cloride dup, likely due to a small chloride value. 
  TMP_FW_PW_C6_15cm_SO4_20260615_Post had an unusually high chloride value for freshwater" #any notes from the run
...
#file path and name for raw summary data file 
  raw_file_name_cl = "Raw Data/COMPASS_TEMPEST_202606_302-432_Cl.txt"
  raw_file_name_so4 = "Raw Data/COMPASS_TEMPEST_202606_302-432_SO4.txt"
  
#Sample ID Path
  SampleID_path = "Raw Data/TEMPEST_SO4_20260501-20260612_corrected.csv"

#file path and name of processed data file 
  processed_file_name = "Processed Data/COMPASS_TEMPEST_Processed_Cl_SO4_202606_302-432.csv" 
...
```{r Fix Sample IDs entered wrong, include = FALSE}

##Data entered incorrectly
# all_dat$sample_ID <- gsub(Old_ID_1, New_ID_1, all_dat$sample_ID)
...
cl_chks_percent_cv <- (sum(chks_Cl$flag_Cl == "Chloride Check Standard RSD within Range - PROCEED", na.rm = TRUE)/nrow(chks_Cl[!is.na(chks_Cl$flag_Cl), ]))*100
so4_chks_percent_cv <- (sum(chks_S$flag_SO4 == "Sulfate Check Standard RSD within Range - PROCEED", na.rm = TRUE)/nrow(chks_S[!is.na(chks_S$flag_SO4), ]))*100
...
all_dat$Dilution <-  ifelse(all_dat$sample_ID == "90", 100, all_dat$Dilution)
all_dat$Dilution <-  ifelse(all_dat$sample_ID == "92", 100, all_dat$Dilution)
all_dat$Dilution <-  ifelse(all_dat$sample_ID == "93", 100, all_dat$Dilution)
all_dat$Dilution <-  ifelse(all_dat$sample_ID == "94", 100, all_dat$Dilution)
all_dat$Dilution <-  ifelse(all_dat$sample_ID == "129", 100, all_dat$Dilution)
all_dat$Dilution <-  ifelse(all_dat$sample_ID == "132", 100, all_dat$Dilution)

all_dat$Dilution <-  ifelse(all_dat$sample_ID == "169", 100, all_dat$Dilution)
all_dat$Dilution <-  ifelse(all_dat$sample_ID == "171", 100, all_dat$Dilution)
all_dat$Dilution <-  ifelse(all_dat$sample_ID == "172", 100, all_dat$Dilution)
all_dat$Dilution <-  ifelse(all_dat$sample_ID == "205", 100, all_dat$Dilution)
all_dat$Dilution <-  ifelse(all_dat$sample_ID == "207", 100, all_dat$Dilution)
all_dat$Dilution <-  ifelse(all_dat$sample_ID == "208", 100, all_dat$Dilution)

all_dat$Dilution <-  ifelse(all_dat$sample_ID == "294", 100, all_dat$Dilution)
all_dat$Dilution <-  ifelse(all_dat$sample_ID == "296", 100, all_dat$Dilution)
all_dat$Dilution <-  ifelse(all_dat$sample_ID == "297", 100, all_dat$Dilution)
...
all_data_flagged <- dat_flagged %>%
  ...
  filter(!str_detect(sample_ID, "SPK")) %>%
  filter(!str_detect(sample_ID, "FATE"))

#Merge with data
Sample_data_IDs <- all_data_flagged %>%
  rename(ID = sample_ID) %>%
...
#remove rerun runoff sample 209
...
final_data <- merged_data %>% 
  ...
  select(
    Project, Sample_ID, Number_on_vial, Volume_mL, Collection_Date_YYYYMMDD, Evacuation_date_YYYYMMDD, Collection_Start_Time_24hrs, Collection_End_Time_24hrs, EST_EDT, 
