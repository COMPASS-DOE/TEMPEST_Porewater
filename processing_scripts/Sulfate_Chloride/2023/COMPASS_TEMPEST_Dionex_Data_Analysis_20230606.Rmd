...
Sdat <- read.table("Raw Data/COMPASS_TEMPEST_20230606_SO4a.txt",sep='\t' , header=T, skip=3)
head(Sdat)

## Only keep the columns that we need 
Sdat <- Sdat[ ,c(1,2,5,7)] # dont need this here 
head(Sdat)


## Name the columns correctly 
colnames(Sdat) <- c("Number", "Sample_ID", "SO4_ppm", "SO4_Area")
...
#Chloride data 
## Read in raw data file from Dionex - copied and saved as a txt
Cldat <- read.table("Raw Data/COMPASS_TEMPEST_20230606_Cla.txt",sep='\t' , header=T, skip=3)
head(Cldat)

## Only keep the columns that we need 
Cldat <- Cldat[ ,c(1,2,5,7)]
head(Cldat)

## Name the columns correctly 
colnames(Cldat) <- c("Number", "Sample_ID", "Cl_ppm", "Cl_Area")
...
all_dat1 <- merge(Sdat, Cldat, by=c("Sample_ID", "Number"))
head(all_dat)

##Remove duplicates - select the first one
all_dat <- all_dat %>%
  arrange(Sample_ID, Number) %>%
...
Perc_dups <- Perc_dups %>% 
    filter(!is.na(SO4_dups_flag))
Perc_dups1 <- QAdups %>% 
  group_by(Cl_dups_flag) %>%
  summarise(Cl_no_rows = length(Cl_dups_flag))
Perc_dups1 <- Perc_dups1 %>% 
    filter(!is.na(Cl_dups_flag))
...
```{r}
##Merge sample data with dilution data
Dilutions <- read.csv("Raw Data/TMP 2023 Sample Dilutions from Dionex.csv")
sampledat_dils <- merge(sampledat, Dilutions, by = "Sample_ID")
unique(sampledat_dils$Dilution)
...
#put it back together with the old data set and look for duplicates 
QAspks <- merge(sampledat_dils, spks)
head(QAspks)
...
# QAspks$Dilution <- 1
# QAspks$Dilution <-  ifelse(str_detect(QAspks$Sample_ID, "TEMPEST"), 50, QAspks$Dilution)
...
unique(QAspks$Dilution)

QAspks$SampleVol <- 1
QAspks$SampleVol <-  ifelse(QAspks$Dilution == 50, 1483, QAspks$SampleVol)
QAspks$SampleVol <-  ifelse(QAspks$Dilution == 56.85, 1483, QAspks$SampleVol)
QAspks$SampleVol <-  ifelse(QAspks$Dilution == 51.16, 1483, QAspks$SampleVol)
QAspks$SampleVol <-  ifelse(QAspks$Dilution == 49.27, 1483, QAspks$SampleVol)
QAspks$SampleVol <-  ifelse(QAspks$Dilution == 56.51, 1483, QAspks$SampleVol)
QAspks$SampleVol <-  ifelse(QAspks$Dilution == 5.15, 1500, QAspks$SampleVol)

# QAspks$SampleVol <-  ifelse(str_detect(QAspks$Sample_ID, "TEMPEST"), 1483, QAspks$SampleVol)
...
sampledat_dils$SO4_ugmL <- ((sampledat_dils$SO4_Area)-SO4_Int)/SO4_Slope
sampledat_dils$Cl_ugmL <- (sampledat_dils$Cl_Area-Cl_Int)/Cl_Slope
head(sampledat_dils)
...
spks$Sample_ID<-gsub("_spk","",as.character(spks$Sample_ID))
colnames(spks) <- c('Sample_ID', 'Dilution', 'SO4_ugmL_spk')
head(spks)
...
QAspks$SampleVol <- 1
QAspks$SampleVol <-  ifelse(QAspks$Dilution == 50, 1483, QAspks$SampleVol)
QAspks$SampleVol <-  ifelse(QAspks$Dilution == 1.1, 1500, QAspks$SampleVol)
QAspks$SampleVol <-  ifelse(QAspks$Dilution == 5.15, 1500, QAspks$SampleVol)
QAspks$SampleVol <-  ifelse(QAspks$Dilution == 4.53, 1500, QAspks$SampleVol)
QAspks$SampleVol <-  ifelse(QAspks$Dilution == 56.51, 1483, QAspks$SampleVol)

# QAspks$SampleVol <-  ifelse(str_detect(QAspks$Sample_ID, "TEMPEST"), 1501, QAspks$SampleVol)
# QAspks$SampleVol <-  ifelse(str_detect(QAspks$Sample_ID, "MSM_UP"), 1501, QAspks$SampleVol)
# QAspks$SampleVol <-  ifelse(str_detect(QAspks$Sample_ID, "MSM_TR"), 1501, QAspks$SampleVol)
# QAspks$SampleVol <-  ifelse(str_detect(QAspks$Sample_ID, "MSM_WC"), 1475, QAspks$SampleVol)
...
write.csv(alldat, file="Processed Data/COMPASS_TEMPEST_Dionex_ClSo4_Final_20230606.csv")           #Change file name
