...
no_meta_flags <- c("TMP_SW_H6_20230612", "TMP_FW_F6_20230606")
```
...
all_data3 <- all_data2 %>% 
  select("Sample_ID", "SO4_ppm", "SO4_Area", "Cl_ppm", "Cl_Area", "SO4_mM", "Cl_mM", "salinity", "Time", "Project", "Plot", "Grid", "Date", "Time_of_day", "Event_Time")

all_data3$"Depth_cm" <- ifelse(all_data3$Grid == "SOURCEWATER", NA, "15")
...
##Add day to 202311 samples
all_data3$Sample_ID <- gsub("202311", "20231116", all_data3$Sample_ID)
all_data3$Date <- gsub("202311", "20231116", all_data3$Date)

##Fix TMP_SW_E5_20230606
all_data3$Sample_ID <- gsub("TMP_SW_E5_20230606", "TMP_SW_E3_20230606", all_data3$Sample_ID)
all_data3$Grid <- ifelse(all_data3$Sample_ID == "TMP_SW_E3_20230606", "E3", all_data3$Grid)
...
dionex_metadata <- dionex_metadata %>%
  rename(
    ...
    Event_Time_meta = Event_Time, 
    Time_of_day_meta = Time_of_day
  )
...
data_plotting$Plot = factor(data_plotting$Plot, levels = c("C", "FW", "SW", "ESTUARY"))
...
merged_data$Grid <- ifelse(merged_data$Grid == "SOURCEWATER", "Source", merged_data$Grid)

final_data <- merged_data %>%
  mutate(Run_notes = ifelse(Sample_ID %in% no_meta_flags, "Sample did not have associated metadata after checking field datasheets", NA)) %>%
  mutate(
    SO4_Conc_flag = NA, 
    SO4_QAQC_flag = NA, 
    Cl_Conc_flag = NA, 
  ...
  select(
         Sample_ID, Project, Plot, Grid, Depth_cm, Event_Time, Time_of_day, 
         Volume_mL, Collection_Date_YYYYMMDD, Evacuation_Date_YYYYMMDD, 
